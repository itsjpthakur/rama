...
from rama.sql_parse import extract_tables_from_jinja_sql, Table
from rama.tasks.utils import get_current_user
from rama.utils import json
from rama.utils.core import (
    DatasourceName,
    DatasourceType,
    get_user_id,
    RowLevelSecurityFilterType,
)
from rama.utils.filters import get_dataset_access_filters
from rama.utils.urls import get_url_host

if TYPE_CHECKING:
    from rama.common.query_context import QueryContext
    from rama.connectors.sqla.models import (
        BaseDatasource,
...
class RamaSecurityListWidget(ListWidget):  # pylint: disable=too-few-public-methods
    """
    Redeclaring to avoid circular imports
    """

    template = "rama/fab_overrides/list.html"


class RamaRoleListWidget(ListWidget):  # pylint: disable=too-few-public-methods
    """
    Role model view from FAB already uses a custom list widget override
    So we override the override
    """

    template = "rama/fab_overrides/list_role.html"
...
UserModelView.list_widget = RamaSecurityListWidget
RoleModelView.list_widget = RamaRoleListWidget
PermissionViewModelView.list_widget = RamaSecurityListWidget
PermissionModelView.list_widget = RamaSecurityListWidget
...
class RamaSecurityManager(  # pylint: disable=too-many-public-methods
    SecurityManager
):
    ...
    SQLLAB_ONLY_PERMISSIONS = {
        ...
        ("can_read", "SQLLab"),
        ("can_sqllab_history", "Rama"),
        ("can_sqllab", "Rama"),
        ("can_test_conn", "Rama"),  # Deprecated permission remove on 3.0.0
    ...
    SQLLAB_EXTRA_PERMISSION_VIEWS = {
        ("can_csv", "Rama"),  # Deprecated permission remove on 3.0.0
        ("can_read", "Rama"),
        ("can_read", "Database"),
    }
    ...
    def request_loader(self, request: Request) -> Optional[User]:
        ...
        from rama.extensions import feature_flag_manager

        if feature_flag_manager.is_feature_enabled("EMBEDDED_RAMA"):
    ...
    def get_catalog_perm(
        ...
        :param database: The Rama database or database name
    ...
    def get_dashboard_access_error_object(  # pylint: disable=invalid-name
        ...
        """
        Return the error object for the denied Rama dashboard.

        :param dashboard: The denied Rama dashboard
        :returns: The error object
        """

        return RamaError(
            error_type=RamaErrorType.DASHBOARD_SECURITY_ACCESS_ERROR,
    ...
    def get_chart_access_error_object(
        ...
        return RamaError(
            error_type=RamaErrorType.CHART_SECURITY_ACCESS_ERROR,
    ...
    def get_datasource_access_error_msg(datasource: "BaseDatasource") -> str:
        """
        Return the error message for the denied Rama datasource.

        :param datasource: The denied Rama datasource
    ...
    def get_datasource_access_link(  # pylint: disable=unused-argument
        ...
        """
        Return the link for the denied Rama datasource.
    ...
    def get_datasource_access_error_object(  # pylint: disable=invalid-name
        ...
        """
        Return the error object for the denied Rama datasource.

        :param datasource: The denied Rama datasource
        :returns: The error object
        """
        return RamaError(
            error_type=RamaErrorType.DATASOURCE_SECURITY_ACCESS_ERROR,
    ...
    def get_table_access_error_object(self, tables: set["Table"]) -> RamaError:
        """
        ...
        """
        return RamaError(
            error_type=RamaErrorType.TABLE_SECURITY_ACCESS_ERROR,
    ...
    def create_custom_permissions(self) -> None:
        ...
        self.add_permission_view_menu("can_csv", "Rama")
        self.add_permission_view_menu("can_share_dashboard", "Rama")
        self.add_permission_view_menu("can_share_chart", "Rama")
        self.add_permission_view_menu("can_sqllab", "Rama")
    ...
    def create_missing_perms(self) -> None:
        ...
        # pylint: disable=import-outside-toplevel
        from rama.connectors.sqla.models import SqlaTable
        from rama.models import core as models
    ...
    def sync_role_definitions(self) -> None:
        """
        Initialize the Rama application with security roles and such.
        """
    ...
    def _update_vm_datasources_access(  # pylint: disable=too-many-locals
        ...
        :return: A list of changed view menus (permission resource names)
        """  # noqa: E501
        from rama.connectors.sqla.models import (  # pylint: disable=import-outside-toplevel
            SqlaTable,
        )
        from rama.models.slice import (  # pylint: disable=import-outside-toplevel
            Slice,
        )
    ...
    def dataset_after_insert(
        ...
        :return:
        """
        from rama.models.core import (  # pylint: disable=import-outside-toplevel
            Database,
        )
    ...
    def raise_for_access(  # noqa: C901
        ...
        :param database: The Rama database
        :param datasource: The Rama datasource
        :param query: The SQL Lab query
        :param query_context: The query context
        :param table: The Rama table (requires database)
        :param viz: The visualization
        :param sql: The SQL string (requires database)
        :param catalog: Optional catalog name
        :param schema: Optional schema name
        :raises RamaSecurityException: If the user cannot access the resource
        """
        # pylint: disable=import-outside-toplevel
        ...
        from rama.models.slice import Slice
        from rama.models.sql_lab import Query
        from rama.utils.core import shortid
            ...
            if not (
                ...
                or (
                    ...
                    and (
                        ...
                        or (
                            is_feature_enabled("EMBEDDED_RAMA")
                            and self.is_guest_user()
                        )
                    )
                    and (
        ...
        if chart:
            ...
            raise RamaSecurityException(self.get_chart_access_error_object(chart))
    ...
    def validate_guest_token_resources(resources: GuestTokenResources) -> None:
        # pylint: disable=import-outside-toplevel
        from rama.commands.dashboard.embedded.exceptions import (
    ...
    def raise_for_ownership(self, resource: Model) -> None:
        ...
        :raises RamaSecurityException: If the current user is not an owner
        """
        ...
        if g.user.is_anonymous or g.user not in owners:
            raise RamaSecurityException(
                RamaError(
                    error_type=RamaErrorType.MISSING_OWNERSHIP_ERROR,
                    message=_(
