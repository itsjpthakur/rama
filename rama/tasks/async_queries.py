...
from rama.exceptions import RamaVizException
...
from rama.views.utils import get_datasource_info, get_viz
    ...
    with override_user(_load_user_from_job_metadata(job_metadata), force=False):
        try:
            ...
            if viz_obj.has_error(payload):
                raise RamaVizException(errors=payload["errors"])
            ...
            result_url = f"/rama/explore_json/data/{cache_key}"
            async_query_manager.update_job(
                job_metadata,
                async_query_manager.STATUS_DONE,
                result_url=result_url,
            ...
            if isinstance(ex, RamaVizException):
                errors = ex.errors
            else:
