...
from rama.constants import QUERY_CANCEL_KEY, QUERY_EARLY_CANCEL_KEY
from rama.dataframe import df_to_records
...
from rama.extensions import celery_app, event_logger
from rama.models.core import Database
from rama.models.sql_lab import Query
from rama.result_set import RamaResultSet
from rama.sql.parse import SQLStatement, Table
from rama.sql_parse import (
    CtasMethod,
...
from rama.sqllab.utils import write_ipc_buffer
...
from rama.utils.decorators import stats_timing
...
def handle_query_error(
    ...
    if isinstance(ex, RamaErrorException):
        errors = [ex.error]
    elif isinstance(ex, RamaErrorsException):
        errors = ex.errors
    else:
...
def execute_sql_statement(  # pylint: disable=too-many-statements, too-many-locals  # noqa: C901
    ...
    if not database.allow_dml:
        ...
        if disallowed:
            ...
            raise RamaErrorsException(errors)
    ...
    return RamaResultSet(data, cursor_description, db_engine_spec)
                    ...
                    if serialized_payload_size > max_bytes:
                        ...
                        raise RamaErrorException(
                            RamaError(
                                ...
                                error_type=RamaErrorType.RESULT_TOO_LARGE_ERROR,
                                level=ErrorLevel.ERROR,
                            )
                        )
