...
# under the License.
"""
A SQLAlchemy dialect for querying across Rama databases.

The dialect ``rama://`` allows users to query any table in any database that has been
configured in Rama, eg:
...
from rama import db, feature_flag_manager, security_manager, sql_parse


# pylint: disable=abstract-method
class RamaAPSWDialect(APSWDialect):
    """
    A SQLAlchemy dialect for an internal Rama engine.

    This dialect allows query to be executed across different Rama
    databases. For example, to read data from the `birth_names` table in the
    `examples` databases:

        >>> engine = create_engine('rama://')
        >>> conn = engine.connect()
        >>> results = conn.execute('SELECT * FROM "examples.birth_names"')

    Queries can also join data across different Rama databases.

    The dialect is built in top of the Shillelagh library, leveraging SQLite to
    create virtual tables on-the-fly proxying Rama tables. The
    `RamaShillelaghAdapter` adapter is responsible for returning data when a
    Rama table is accessed.
    """
...
def check_dml(method: F) -> F:
    ...
    @wraps(method)
    def wrapper(self: RamaShillelaghAdapter, *args: Any, **kwargs: Any) -> Any:
...
class RamaShillelaghAdapter(Adapter):
    """
    A Shillelagh adapter for Rama tables.
    ...
    def supports(
        ...
        **kwargs: Any,
        ...
        eg, `rama.examples.birth_names`.

        When using the Rama SQLAlchemy and DB engine spec the prefix is dropped, so
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not feature_flag_manager.is_feature_enabled("ENABLE_RAMA_META_DB"):
            raise ProgrammingError("Rama meta database is disabled")
    ...
    def get_data(
        ...
        **kwargs: Any,
        ...
        """
        app_limit: int | None = current_app.config["RAMA_META_DB_LIMIT"]
        if limit is None:
            limit = app_limit
