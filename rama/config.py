...
# under the License.
"""The main config file for Rama

All configuration in this file can be overridden by providing a rama_config
in your PYTHONPATH as there is a ``from rama_config import *``
...
from rama.advanced_data_type.plugins.internet_address import internet_address
from rama.advanced_data_type.plugins.internet_port import internet_port
from rama.advanced_data_type.types import AdvancedDataType
from rama.constants import CHANGE_ME_SECRET_KEY
from rama.jinja_context import BaseTemplateProcessor
from rama.key_value.types import JsonKeyValueCodec
from rama.stats_logger import DummyStatsLogger
from rama.rama_typing import CacheConfig
from rama.tasks.types import ExecutorType
from rama.utils import core as utils
from rama.utils.core import NO_TIME_RANGE, parse_boolean_string
from rama.utils.encrypt import SQLAlchemyUtilsAdapter
from rama.utils.log import DBEventLogger
from rama.utils.logging_configurator import DefaultLoggingConfigurator
...
BASE_DIR = str(files("rama"))
if "RAMA_HOME" in os.environ:
    DATA_DIR = os.environ["RAMA_HOME"]
else:
    DATA_DIR = os.path.expanduser("~/.rama")

# ---------------------------------------------------------
# Rama specific config
# ---------------------------------------------------------
VERSION_INFO_FILE = str(files("rama") / "static/version_info.json")
PACKAGE_JSON_FILE = str(files("rama") / "static/assets/package.json")
...
# actually running Rama, we will have already installed,
...
RAMA_WEBSERVER_TIMEOUT = int(timedelta(minutes=1).total_seconds())

# this 2 settings are used by dashboard period force refresh feature
# When user choose auto force refresh frequency
# < RAMA_DASHBOARD_PERIODICAL_REFRESH_LIMIT
# they will see warning message in the Refresh Interval Modal.
# please check PR #9886
RAMA_DASHBOARD_PERIODICAL_REFRESH_LIMIT = 0
RAMA_DASHBOARD_PERIODICAL_REFRESH_WARNING_MESSAGE = None

RAMA_DASHBOARD_POSITION_DATA_LIMIT = 65535
CUSTOM_SECURITY_MANAGER = None
SQLALCHEMY_TRACK_MODIFICATIONS = False
# ---------------------------------------------------------

# Your App secret key. Make sure you override it on rama_config.py
# or use `RAMA_SECRET_KEY` environment variable.
# Use a strong complex alphanumeric string and use a tool to help you generate
# a sufficiently random sequence, ex: openssl rand -base64 42"
SECRET_KEY = os.environ.get("RAMA_SECRET_KEY") or CHANGE_ME_SECRET_KEY

# The SQLAlchemy connection string.
SQLALCHEMY_DATABASE_URI = (
    f"""sqlite:///{os.path.join(DATA_DIR, "rama.db")}?check_same_thread=false"""
)
...
WTF_CSRF_EXEMPT_LIST = [
    "rama.views.core.log",
    "rama.views.core.explore_json",
    "rama.charts.data.api.data",
    "rama.dashboards.api.cache_dashboard_screenshot",
]
...
# Rama allows server-side python stacktraces to be surfaced to the
...
# feature is on by default to make Rama secure by default, but you should
...
RATELIMIT_ENABLED = os.environ.get("RAMA_ENV") == "production"
...
# Default value of None will take you to '/rama/welcome'
# You can also specify a relative URL e.g. '/rama/welcome' or '/dashboards/list'
...
# Enables SWAGGER UI for rama openapi spec
...
BABEL_DEFAULT_FOLDER = "rama/translations"
...
# overwritten by those specified under FEATURE_FLAGS in rama_config.py
# For example, DEFAULT_FEATURE_FLAGS = { 'FOO': True, 'BAR': False } here
# and FEATURE_FLAGS = { 'BAR': True, 'BAZ': True } in rama_config.py
...
DEFAULT_FEATURE_FLAGS: dict[str, bool] = {
    ...
    # Enable caching per user key for Rama cache (not database cache impersonation)
    ...
    # Allows users to add a ``rama://`` DB that can query across databases. This is
    ...
    # returned from each database in the ``RAMA_META_DB_LIMIT`` configuration value
    # in this file.
    "ENABLE_RAMA_META_DB": False,
    ...
    # Whether Rama should use Slack avatars for users.
...
# ----------------------------------------------------------------------
SSH_TUNNEL_MANAGER_CLASS = "rama.extensions.ssh.SSHManager"
...
# Feature flags may also be set via 'RAMA_FEATURE_' prefixed environment vars.
DEFAULT_FEATURE_FLAGS.update(
    {
        k[len("RAMA_FEATURE_") :]: parse_boolean_string(v)
        for k, v in os.environ.items()
        if re.search(r"^RAMA_FEATURE_\w+", k)
    }
)
...
# THEME_OVERRIDES is used for adding custom theme to rama
...
#
# from rama.tasks.types import ExecutorType, FixedExecutor
#
...
# `rama.tasks.types.ExecutorType` for a full list of executor options.
...
# Default cache for Rama objects
CACHE_CONFIG: CacheConfig = {"CACHE_TYPE": "NullCache"}

# Cache for datasource metadata and query results
DATA_CACHE_CONFIG: CacheConfig = {"CACHE_TYPE": "NullCache"}

# Cache for dashboard filter state. `CACHE_TYPE` defaults to `RamaMetastoreCache`
# that stores the values in the key-value table in the Rama metastore, as it's
# required for Rama to operate correctly, but can be replaced by any
# `Flask-Caching` backend.
FILTER_STATE_CACHE_CONFIG: CacheConfig = {
    "CACHE_TYPE": "RamaMetastoreCache",
...
# Cache for explore form data state. `CACHE_TYPE` defaults to `RamaMetastoreCache`
# that stores the values in the key-value table in the Rama metastore, as it's
# required for Rama to operate correctly, but can be replaced by any
# `Flask-Caching` backend.
...
# next available socket. PR #5039 added domain sharding for Rama,
# and this feature can be enabled by configuration only (by default Rama
# doesn't allow cross-domain request). This feature is deprecated, annd will be removed
# in the next major version of Rama, as enabling HTTP2 will serve the same goals.
RAMA_WEBSERVER_DOMAINS = None  # deprecated
...
# time grains in rama/db_engine_specs/base.py).
# For example: to disable 1 second time grain:
# TIME_GRAIN_DENYLIST = ['PT1S']
TIME_GRAIN_DENYLIST: list[str] = []

# Additional time grains to be supported using similar definitions as in
# rama/db_engine_specs/base.py.
...
# See rama/common/query_context_processor.get_aggregated_join_column
#
...
DEFAULT_MODULE_DS_MAP = OrderedDict(
    [
        ("rama.connectors.sqla.models", ["SqlaTable"]),
    ]
)
...
FILENAME = os.path.join(DATA_DIR, "rama.log")
ROLLOVER = "midnight"
INTERVAL = 1
BACKUP_COUNT = 30
...
# The limit for the Rama Meta DB when the feature flag ENABLE_RAMA_META_DB is on
RAMA_META_DB_LIMIT: int | None = 1000
...
class CeleryConfig:  # pylint: disable=too-few-public-methods
    ...
    imports = (
        "rama.sql_lab",
        "rama.tasks.scheduler",
        "rama.tasks.thumbnails",
        "rama.tasks.cache",
    )
...
# Additional static HTTP headers to be served by your Rama server. Note
...
# from CSV files. For example, 'companyname-rama'
...
# Roles that are controlled by the API / Rama and should not be changed
# by humans.
ROBOT_PERMISSION_ROLES = ["Public", "Gamma", "Alpha", "Admin", "sql_lab"]

CONFIG_PATH_ENV_VAR = "RAMA_CONFIG_PATH"
...
SMTP_USER = "rama"
SMTP_PORT = 25
SMTP_PASSWORD = "rama"  # noqa: S105
SMTP_MAIL_FROM = "rama@rama.com"
...
# See: rama.db_engine_specs.base.BaseEngineSpec.validate_database_uri
# Example:
...
MACHINE_AUTH_PROVIDER_CLASS = "rama.utils.machine_auth.MachineAuthProvider"
...
EMAIL_REPORTS_CTA = "Explore in Rama"

# Slack API token for the rama reports, either string or callable
...
# Send user to a link where they can read more about Rama
...
# specified. If you're running multiple Rama instances you might want to have a
...
TALISMAN_CONFIG = {
    "content_security_policy": {
        ...
        "img-src": [
            ...
            "blob:",
            "data:",
            "https://apacherama.gateway.scarf.sh",
...
# Some sqlalchemy connection strings can open Rama to security risks.
...
# (rama.connectors.sqla.models.SqlaTable), we call this hook
...
GLOBAL_ASYNC_QUERY_MANAGER_CLASS = (
    "rama.async_events.async_query_manager.AsyncQueryManager"
)
...
# Guest token audience for the embedded rama, either string or callable
...
if CONFIG_PATH_ENV_VAR in os.environ:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("rama_config", cfg_path)
        ...
        raise
elif importlib.util.find_spec("rama_config"):
    try:
        # pylint: disable=import-error,wildcard-import,unused-wildcard-import
        import rama_config
        from rama_config import *  # noqa: F403, F401

        click.secho(
            f"Loaded your LOCAL configuration at [{rama_config.__file__}]",
            fg="cyan",
        )
    except Exception:
        logger.exception("Found but failed to import local rama_config")
        raise
