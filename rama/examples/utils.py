...
from rama.commands.importers.v1.examples import ImportExamplesCommand
from rama.commands.importers.v1.utils import METADATA_FILE_NAME
...
def load_examples_from_configs(
    ...
    """
    Load all the examples inside rama/examples/configs/.
    """
...
def load_contents(load_test_data: bool = False) -> dict[str, Any]:
    ...
    root = files("rama") / "examples/configs"
    resource_names = (files("rama") / str(root)).iterdir()
    ...
    while queue:
        ...
        if (files("rama") / str(path_name)).is_dir():
            queue.extend(
                path_name / str(child_name)
                for child_name in (files("rama") / str(path_name)).iterdir()
            )
