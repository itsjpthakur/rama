...
from rama.daos.database import DatabaseDAO, SSHTunnelDAO
...
from rama.utils.ssh_tunnel import unmask_password_info
    ...
    def run(self) -> None:  # pylint: disable=too-many-statements,too-many-branches  # noqa: C901
        ...
        try:
            ...
            with database.get_sqla_engine(override_ssh_tunnel=ssh_tunnel) as engine:
                ...
                except RamaTimeoutException as ex:
            ...
            raise RamaErrorsException(errors) from ex
        ...
        except (RamaTimeoutException, SSHTunnelingNotEnabledError) as ex:
            event_logger.log_with_context(
                action=get_log_connection_action(
                    "test_connection_error", ssh_tunnel, ex
                ),
