...
from rama.exceptions import RamaErrorException, RamaErrorsException
...
class DatabaseTestConnectionFailedError(RamaErrorsException):
    status = 422
    message = _("Connection failed, please check your connection settings")
...
class DatabaseTestConnectionUnexpectedError(RamaErrorsException):
    status = 422
...
class NoValidatorConfigFoundError(RamaErrorException):
    status = 422
    message = _("no SQL validator is configured")


class NoValidatorFoundError(RamaErrorException):
    status = 422
    message = _("No validator found (configured for the engine)")


class ValidatorSQLError(RamaErrorException):
    status = 422
...
class ValidatorSQL400Error(RamaErrorException):
    status = 400
    message = _("Was unable to check your query")
...
class InvalidEngineError(RamaErrorException):
    status = 422


class DatabaseOfflineError(RamaErrorException):
    status = 422


class InvalidParametersError(RamaErrorsException):
    status = 422
