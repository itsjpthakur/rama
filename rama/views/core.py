...
from rama.async_events.async_query_manager import AsyncQueryTokenException
from rama.commands.chart.exceptions import ChartNotFoundError
from rama.commands.chart.warm_up_cache import ChartWarmUpCacheCommand
from rama.commands.dashboard.exceptions import DashboardAccessDeniedError
from rama.commands.dashboard.permalink.get import GetDashboardPermalinkCommand
from rama.commands.dataset.exceptions import DatasetNotFoundError
from rama.commands.explore.form_data.create import CreateFormDataCommand
from rama.commands.explore.form_data.get import GetFormDataCommand
from rama.commands.explore.form_data.parameters import CommandParameters
from rama.commands.explore.permalink.get import GetExplorePermalinkCommand
from rama.common.chart_data import ChartDataResultFormat, ChartDataResultType
from rama.connectors.sqla.models import BaseDatasource, SqlaTable
from rama.daos.chart import ChartDAO
from rama.daos.datasource import DatasourceDAO
from rama.dashboards.permalink.exceptions import DashboardPermalinkGetFailedError
...
from rama.extensions import async_query_manager, cache_manager
...
from rama.models.user_attributes import UserAttribute
from rama.rama_typing import FlaskResponse
from rama.utils import core as utils, json
from rama.utils.cache import etag_cache
...
class Rama(BaseRamaView):
    """The base views for Rama!"""
    ...
    def explore_json(
        ...
        if (
            ...
            and not security_manager.can_access("can_csv", "Rama")
        ):
    ...
    def get_redirect_url() -> str:
        ...
        to the cache layer.
        """
        redirect_url = request.url.replace("/rama/explore", "/explore")
        form_data_key = None
    ...
    def explore(  # noqa: C901
        ...
        if request.method == "GET":
            return redirect(Rama.get_redirect_url())
        ...
        slice_download_perm = security_manager.can_access("can_csv", "Rama")
        ...
        except (RamaException, SQLAlchemyError):
    ...
    def dashboard_permalink(
        ...
        url = f"/rama/dashboard/{dashboard_id}?permalink_key={key}"
    ...
    def welcome(self) -> FlaskResponse:
        ...
        if not g.user or not get_user_id():
            if conf["PUBLIC_ROLE_LIKE"]:
                return self.render_template("rama/public_welcome.html")
