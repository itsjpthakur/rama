...
from rama.exceptions import RamaParseError
from rama.sql.dialects.firebolt import Firebolt
...
# engines supported by Rama this is `sqlglot.exp.Expression`, but there is a special
...
class BaseSQLStatement(Generic[InternalRepresentation]):
    ...
    The `engine` parameters comes from the `engine` attribute in a Rama DB engine
    spec.
    """
...
class SQLStatement(BaseSQLStatement[exp.Expression]):
    ...
    def _parse_statement(
        ...
        if len(statements) != 1:
            raise RamaParseError("SQLStatement should have exactly one statement")
...
class KustoKQLStatement(BaseSQLStatement[str]):
    ...
    def _parse_statement(
        ...
        if engine != "kustokql":
            raise RamaParseError(f"Invalid engine: {engine}")
    ...
    def _extract_tables_from_statement(
        ...
        logger.warning(
            ...
            "roles will not be enforced by Rama in the database."
        )
        return set()
...
class SQLScript:
    ...
    # adds a lot of complexity to Rama, so we should avoid adding new engines to
    # this data structure.
    special_engines = {
