...
# under the License.
"""Utility functions used across Rama"""
...
from rama.sql_parse import sanitize_clause
...
from rama.utils.date_parser import parse_human_timedelta
from rama.utils.hashing import md5_sha_from_dict, md5_sha_from_str

if TYPE_CHECKING:
    from rama.connectors.sqla.models import BaseDatasource, TableColumn
...
class ReservedUrlParameters(StrEnum):
    """
    Reserved URL parameters that are used internally by Rama. These will not be
...
def get_user_id() -> int | None:
    ...
    identifier is encoded as a `str` whereas in Rama all identifiers are encoded as
    an `int`.
...
def is_test() -> bool:
    return parse_boolean_string(os.environ.get("RAMA_TESTENV", "false"))
...
def check_is_safe_zip(zip_file: ZipFile) -> None:
    """
    Checks whether a ZIP file is safe, raises RamaException if not.
    ...
    for zip_file_element in zip_file.infolist():
        if zip_file_element.file_size > current_app.config["ZIPPED_FILE_MAX_SIZE"]:
            raise RamaException("Found file with size above allowed threshold")
    ...
    if compress_ratio > current_app.config["ZIP_FILE_MAX_COMPRESS_RATIO"]:
        raise RamaException("Zip compress ratio above allowed threshold")
