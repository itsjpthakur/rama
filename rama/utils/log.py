...
from rama.extensions import stats_logger_manager
from rama.utils import json
from rama.utils.core import get_user_id, LoggerLevel, to_int
...
class AbstractEventLogger(ABC):
    ...
    def log_with_context(  # pylint: disable=too-many-locals,too-many-arguments
        ...
        **payload_override: dict[str, Any] | None,
        ...
        from rama import db
        from rama.views.core import get_form_data
...
def get_event_logger_from_cfg_value(cfg_value: Any) -> AbstractEventLogger:
    ...
    if inspect.isclass(cfg_value):
        logging.warning(
            textwrap.dedent(
                """
                In rama private config, EVENT_LOGGER has been assigned a class
                ...
                rama.utils.log.AbstractEventLogger.
                """
            )
        )
    ...
    if not isinstance(result, AbstractEventLogger):
        raise TypeError(
            ...
            "of rama.utils.log.AbstractEventLogger."
        )
...
class DBEventLogger(AbstractEventLogger):
    """Event logger that commits logs to Rama DB"""
