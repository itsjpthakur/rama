...
class SynchronousSqlJsonExecutor(SqlJsonExecutorBase):
    ...
    def execute(
        ...
        except RamaTimeoutException:
            raise
        except Exception as ex:
            logger.exception("Query %i failed unexpectedly", query_id)
            raise RamaGenericDBErrorException(
        ...
        if data.get("status") == QueryStatus.FAILED:  # type: ignore
            ...
            if data["errors"]:  # type: ignore
                raise RamaErrorsException(
                    [RamaError(**params) for params in data["errors"]]  # type: ignore
                )
            # old string-only error message
            print(data)
            raise RamaGenericDBErrorException(data["error"])  # type: ignore
...
class ASynchronousSqlJsonExecutor(SqlJsonExecutorBase):
    def execute(
        ...
        try:
            ...
            error = RamaError(
                message=message,
                error_type=RamaErrorType.ASYNC_WORKERS_ERROR,
            ...
            raise RamaErrorException(error) from ex
