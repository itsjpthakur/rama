...
with contextlib.suppress(ImportError, RuntimeError):  # pyocient may not be installed
    # Ensure pyocient inherits Rama's logging level
    import geojson
    import pyocient
    ...
    rama_log_level = app.config["LOG_LEVEL"]
    pyocient.logger.setLevel(rama_log_level)

from rama.constants import TimeGrain
from rama.db_engine_specs.base import BaseEngineSpec
from rama.errors import RamaErrorType
...
def _linestring_to_geo_json(
    ...
    compatible with the Rama visualization toolkit (powered
    by Deck.gl).
...
# that cannot be processed natively by Rama.
#
# Rama serializes temporal objects using a custom serializer
# defined in rama/utils/core.py (#json_int_dttm_ser(...)). Other
...
def _find_columns_to_sanitize(cursor: Any) -> list[PlacedSanitizeFunc]:
    """
    Cleans the column value for consumption by Rama.
...
class OcientEngineSpec(BaseEngineSpec):
    ...
    cte_alias = "cte__"
    # Store mapping of rama Query id -> Ocient ID
    ...
    custom_errors: dict[Pattern[str], tuple[str, RamaErrorType, dict[str, Any]]] = {
        CONNECTION_INVALID_USERNAME_REGEX: (
            __('The username "%(username)s" does not exist.'),
            RamaErrorType.CONNECTION_INVALID_USERNAME_ERROR,
            {},
        ),
        CONNECTION_INVALID_PASSWORD_REGEX: (
            ...
            RamaErrorType.CONNECTION_INVALID_PASSWORD_ERROR,
            {},
        ),
        CONNECTION_UNKNOWN_DATABASE_REGEX: (
            __('Could not connect to database: "%(database)s"'),
            RamaErrorType.CONNECTION_UNKNOWN_DATABASE_ERROR,
            {},
        ),
        CONNECTION_INVALID_HOSTNAME_REGEX: (
            __('Could not resolve hostname: "%(host)s".'),
            RamaErrorType.CONNECTION_INVALID_HOSTNAME_ERROR,
            {},
        ),
        CONNECTION_INVALID_PORT_ERROR: (
            __("Port out of range 0-65535"),
            RamaErrorType.CONNECTION_INVALID_PORT_ERROR,
            {},
        ),
        ...
        COLUMN_DOES_NOT_EXIST_REGEX: (
            ...
            RamaErrorType.COLUMN_DOES_NOT_EXIST_ERROR,
            {},
        ),
    }
    ...
    def fetch_data(
        ...
        try:
            ...
            with OcientEngineSpec.query_id_mapping_lock:
                del OcientEngineSpec.query_id_mapping[cursor.rama_query_id]
            raise
    ...
    def get_cancel_query_id(cls, cursor: Any, query: Query) -> Optional[str]:
        ...
        # If None is returned, Rama will not call cancel_query
        return "DUMMY_VALUE"
