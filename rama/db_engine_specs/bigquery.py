...
from rama.databases.schemas import encrypted_field_properties, EncryptedString
from rama.databases.utils import make_url_safe
from rama.db_engine_specs.base import BaseEngineSpec, BasicPropertiesType
from rama.db_engine_specs.exceptions import RamaDBAPIConnectionError
from rama.errors import RamaError, RamaErrorType
from rama.exceptions import RamaException
from rama.sql.parse import SQLScript
from rama.sql_parse import Table
from rama.rama_typing import ResultSetColumnType
...
if TYPE_CHECKING:
    from rama.models.core import Database  # pragma: no cover
...
class BigQueryEngineSpec(BaseEngineSpec):  # pylint: disable=too-many-public-methods
    ...
    custom_errors: dict[Pattern[str], tuple[str, RamaErrorType, dict[str, Any]]] = {
        CONNECTION_DATABASE_PERMISSIONS_REGEX: (
            ...
            RamaErrorType.CONNECTION_DATABASE_PERMISSIONS_ERROR,
            {},
        ),
        ...
        SCHEMA_DOES_NOT_EXIST_REGEX: (
            ...
            RamaErrorType.SCHEMA_DOES_NOT_EXIST_ERROR,
            {},
        ),
        SYNTAX_ERROR_REGEX: (
            __(
    ...
    def df_to_sql(
        ...
        if not table.schema:
            raise RamaException("The table schema must be defined")
    ...
    def _get_client(
        ...
        try:
            ...
            raise RamaDBAPIConnectionError(
    ...
    def estimate_query_cost(  # pylint: disable=too-many-arguments
        ...
        if not cls.get_allow_cost_estimate(extra):
            raise RamaException("Database does not support cost estimation")
    ...
    def get_catalog_names(
        ...
        with database.get_sqla_engine() as engine:
            try:
                client = cls._get_client(engine, database)
            except RamaDBAPIConnectionError:
                logger.warning(
    ...
    def get_dbapi_exception_mapping(cls) -> dict[type[Exception], type[Exception]]:
        ...
        return {DefaultCredentialsError: RamaDBAPIConnectionError}
