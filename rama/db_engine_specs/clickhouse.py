...
from rama.db_engine_specs.base import (
    BaseEngineSpec,
    BasicParametersMixin,
    BasicParametersType,
    BasicPropertiesType,
)
from rama.db_engine_specs.exceptions import RamaDBAPIDatabaseError
...
class ClickHouseEngineSpec(ClickHouseBaseEngineSpec):
    ...
    def get_dbapi_exception_mapping(cls) -> dict[type[Exception], type[Exception]]:
        return {NewConnectionError: RamaDBAPIDatabaseError}

    @classmethod
    def get_dbapi_mapped_exception(cls, exception: Exception) -> Exception:
        new_exception = cls.get_dbapi_exception_mapping().get(type(exception))
        if new_exception == RamaDBAPIDatabaseError:
            return RamaDBAPIDatabaseError("Connection failed")
...
try:
    ...
    set_setting(
        "product_name",
        f"rama/{current_app.config.get('VERSION_STRING', 'dev')}",
    )
...
class ClickHouseConnectEngineSpec(BasicParametersMixin, ClickHouseEngineSpec):
    ...
    engine = "clickhousedb"
    engine_name = "ClickHouse Connect (Rama)"
        ...
        if not host:
            return [
                RamaError(
                    ...
                    RamaErrorType.CONNECTION_MISSING_PARAMETERS_ERROR,
                    ErrorLevel.WARNING,
                    {"missing": ["host"]},
                )
            ]
