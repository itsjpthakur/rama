...
NICE_TO_HAVE_FEATURES = {
    ...
    "dbapi_exception_mapping": "Maps driver exceptions to Rama exceptions",
    "custom_errors": "Parses error messages and returns Rama errors",
...
def diagnose(spec: type[BaseEngineSpec]) -> dict[str, Any]:
    ...
    from rama.sql_validators.postgres import PostgreSQLValidator
    from rama.sql_validators.presto_db import PrestoDBSQLValidator
...
def generate_table() -> list[list[Any]]:
    ...
    info = {k: v for k, v in info.items() if v["module"].startswith("rama")}
...
if __name__ == "__main__":
    from rama.app import create_app
