...
from rama import db, sql_parse
from rama.constants import QUERY_CANCEL_KEY, TimeGrain as TimeGrainConstants
from rama.databases.utils import get_table_metadata, make_url_safe
from rama.errors import ErrorLevel, RamaError, RamaErrorType
from rama.exceptions import DisallowedSQLFunction, OAuth2Error, OAuth2RedirectError
from rama.sql.parse import BaseSQLStatement, SQLScript, Table
from rama.sql_parse import ParsedQuery
from rama.rama_typing import (
    OAuth2ClientConfig,
    OAuth2State,
    OAuth2TokenResponse,
    ResultSetColumnType,
...
from rama.utils.core import ColumnSpec, GenericDataType
from rama.utils.hashing import md5_sha_from_str
from rama.utils.json import redact_sensitive, reveal_sensitive
from rama.utils.network import is_hostname_valid, is_port_open
from rama.utils.oauth2 import encode_oauth2_state

if TYPE_CHECKING:
    from rama.connectors.sqla.models import TableColumn
    from rama.databases.schemas import TableMetadataResponse
...
class BaseEngineSpec:  # pylint: disable=too-many-public-methods
    ...
    custom_errors: dict[
        Pattern[str], tuple[str, RamaErrorType, dict[str, Any]]
    ] = {}
    ...
    def start_oauth2_dance(cls, database: Database) -> None:
        ...
        can authorize Rama to access the database. Once the user has authorized, the
        ...
        # The state is passed to the OAuth2 provider, and sent back to Rama after
        # the user authorizes the access. The redirect endpoint in Rama can then
    ...
    def get_allows_alias_in_select(
        ...
        In Dremio this atribute is version-dependent, so Rama needs to inspect the
    ...
    def get_dbapi_mapped_exception(cls, exception: Exception) -> Exception:
        """
        Get a rama custom DBAPI exception from the driver specific exception.
        ...
        :return: Rama custom DBAPI exception
        """
    ...
    def adjust_engine_params(  # pylint: disable=unused-argument
        ...
        Rama knows in which schema a given query is running in order to enforce
        permissions (see #23385 and #23401).

        Currently, changing the catalog is not supported. The method accepts a catalog so
        that when catalog support is added to Rama the interface remains the same.
    ...
    def get_column_types(
        ...
        :return: SQLAlchemy and generic Rama column types
        """
        if not column_type:
            return None
        ...
        try:
            ...
            errors.append(
                RamaError(
                    ...
                    error_type=RamaErrorType.CONNECTION_INVALID_PORT_ERROR,
                    level=ErrorLevel.ERROR,
                    extra={"invalid": ["port"]},
                ),
            )
        if not (isinstance(port, int) and 0 <= port < 2**16):
            ...
            errors.append(
                RamaError(
                    ...
                    error_type=RamaErrorType.CONNECTION_PORT_CLOSED_ERROR,
                    level=ErrorLevel.ERROR,
                    extra={"invalid": ["port"]},
                ),
            )
