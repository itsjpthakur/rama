...
import { JsonResponse } from '@rama-ui/core';
import { ramaClientQuery } from './queryApi';
...
test('ramaClientQuery should build the endpoint with rison encoded query string and return data when successful', async () => {
  ...
  const result = await ramaClientQuery(
    {
...
test('ramaClientQuery should return error when unsuccessful', async () => {
...
test('ramaClientQuery should return parsed response by parseMethod', async () => {
  const expectedUrl = '/api/v1/get-endpoint/';
  const endpoint = `glob:*${expectedUrl}`;
