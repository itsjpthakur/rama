...
import * as RamaCore from '@rama-ui/core';
import { isFeatureEnabled } from '@rama-ui/core';
import PropertiesModal from '.';

jest.mock('@rama-ui/core', () => ({
  ...jest.requireActual('@rama-ui/core'),
  isFeatureEnabled: jest.fn(),
  getCategoricalSchemeRegistry: jest.fn(() => ({
    keys: () => ['ramaColors'],
    get: () => ['#FFFFFF', '#000000'],
    getDefaultKey: () => 'ramaColors',
  })),
}));
...
describe('PropertiesModal', () => {
  ...
  test('should render - FeatureFlag disabled', async () => {
    ...
    expect(spyColorSchemeControlWrapper).toHaveBeenCalledWith(
      expect.objectContaining({ colorScheme: 'ramaColors' }),
      {},
    );
  });
  ...
  test('submitting with onlyApply:false', async () => {
    const put = jest.spyOn(RamaCore.RamaClient, 'put');
  ...
  test('should show all roles', async () => {
    ...
    const getSelect = () =>
      screen.getByRole('combobox', { name: RamaCore.t('Roles') });
  ...
  test('should show active owners with dashboard rbac', async () => {
    ...
    const open = () => waitFor(() => userEvent.click(getSelect()));
    const getSelect = () =>
      screen.getByRole('combobox', { name: RamaCore.t('Owners') });
    ...
    expect(options[0]).toHaveTextContent('Rama Admin');
  });
