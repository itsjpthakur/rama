...
const setup = overrideProps =>
  mount(
    ...
    </Provider>,
    {
      ...
      wrappingComponentProps: { theme: ramaTheme },
    },
  );
...
describe.skip('PropertiesModal', () => {
  ...
  describe('onColorSchemeChange', () => {
    it('sets up a default state', () => {
      const wrapper = setup({ colorScheme: 'RAMA_DEFAULT' });
      expect(
        wrapper.find('PropertiesModal').instance().state.values.colorScheme,
      ).toEqual('RAMA_DEFAULT');
    });
    describe('with a valid color scheme as an arg', () => {
      describe('without metadata', () => {
        ...
        it('updates the color scheme in the metadata', () => {
          ...
          modalInstance.onColorSchemeChange('RAMA_DEFAULT');
          expect(spy).toHaveBeenCalledWith(
            '{"something": "foo", "color_scheme": "RAMA_DEFAULT", "label_colors": {}}',
          );
        });
      });
      describe('with metadata', () => {
        describe('with color_scheme in the metadata', () => {
          it('will update the metadata', () => {
            ...
            modalInstance.onColorSchemeChange('RAMA_DEFAULT');
            expect(spy).toHaveBeenCalledWith(
              '{"color_scheme": "RAMA_DEFAULT", "label_colors": {}}',
            );
          });
        });
  ...
  describe('fetchDashboardDetails', () => {
    it('should make an api call', () => {
      const spy = jest.spyOn(RamaClient, 'get');
      const wrapper = setup();
      const modalInstance = wrapper.find('PropertiesModal').instance();
    ...
    it('should call onOwnersChange', async () => {
      const wrapper = setup();
      const modalInstance = wrapper.find('PropertiesModal').instance();
      const fetchSpy = jest.spyOn(RamaClient, 'get').mockResolvedValue({
        json: {
          result: {
            dashboard_title: 'New Title',
            slug: '/new',
          ...
          const fetchSpy = jest.spyOn(RamaClient, 'get').mockResolvedValue({
            json: {
              result: {
                dashboard_title: 'New Title',
                slug: '/new',
                json_metadata: '{"color_scheme":"RAMA_DEFAULT"}',
                owners: [],
                roles: [],
              },
            },
