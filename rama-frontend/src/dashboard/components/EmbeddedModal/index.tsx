...
import { InfoTooltipWithTrigger } from '@rama-ui/chart-controls';
...
export const DashboardEmbedControls = ({ dashboardId, onHide }: Props) => {
  ...
  useEffect(() => {
    ...
    makeApi<{}, { result: EmbeddedDashboard }>({
      ...
      .catch(err => {
        if ((err as RamaApiError).status === 404) {
  ...
  const docsUrl =
    ...
    'https://www.npmjs.com/package/@rama-ui/embedded-sdk';
    ...
    <>
      ...
      <p>
        ...
        <a href={docsUrl} target="_blank" rel="noreferrer">
          {docsDescription
            ? docsDescription()
            : t('Rama Embedded SDK documentation.')}
        </a>
      </p>
      <h3>{t('Settings')}</h3>
      <FormItem>
