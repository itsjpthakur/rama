...
const createProps = () => ({
  ...
  url: `/rama/dashboard/${DASHBOARD_ID}`,
  copyMenuItemTitle: 'Copy dashboard URL',
  emailMenuItemTitle: 'Share dashboard by email',
  emailSubject: 'Rama dashboard COVID Vaccine Dashboard',
...
beforeAll((): void => {
  ...
  fetchMock.post(
    ...
    { key: '123', url: 'http://localhost/rama/dashboard/p/123/' },
    {
      sendAsJson: true,
    },
  );
...
test('Click on "Copy dashboard URL" and succeed', async () => {
  ...
  await waitFor(async () => {
    ...
    expect(value).toBe('http://localhost/rama/dashboard/p/123/');
...
test('Click on "Share dashboard by email" and succeed', async () => {
  ...
  await waitFor(() => {
    expect(props.addDangerToast).toHaveBeenCalledTimes(0);
    expect(window.location.href).toBe(
      'mailto:?Subject=Rama%20dashboard%20COVID%20Vaccine%20Dashboard%20&Body=Check%20out%20this%20dashboard%3A%20http%3A%2F%2Flocalhost%2Frama%2Fdashboard%2Fp%2F123%2F',
    );
  });
});
