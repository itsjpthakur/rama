...
import { styled, t, logging } from '@rama-ui/core';
...
const Chart = props => {
  ...
  const ramaCanExplore = useSelector(
    state => !!state.dashboardInfo.rama_can_explore,
  );
  const ramaCanShare = useSelector(
    state => !!state.dashboardInfo.rama_can_share,
  );
  const ramaCanCSV = useSelector(
    state => !!state.dashboardInfo.rama_can_csv,
  );
  const timeout = useSelector(
    state => state.dashboardInfo.common.conf.RAMA_WEBSERVER_TIMEOUT,
  );
  ...
  return (
    ...
    >
      <SliceHeader
        ...
        ramaCanExplore={ramaCanExplore}
        ramaCanShare={ramaCanShare}
        ramaCanCSV={ramaCanCSV}
