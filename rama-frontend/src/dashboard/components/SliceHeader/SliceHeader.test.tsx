...
import { getExtensionsRegistry, VizType } from '@rama-ui/core';
...
jest.mock('src/dashboard/components/SliceHeaderControls', () => ({
  ...
  default: (props: any) => (
    <div
      ...
      data-rama-can-explore={props.ramaCanExplore}
      data-rama-can-csv={props.ramaCanCSV}
...
test('Should render - default props', () => {
  ...
  // @ts-ignore
  delete props.ramaCanExplore;
  // @ts-ignore
  delete props.ramaCanCSV;
...
test('Should render click to edit prompt and run onExploreChart on click', async () => {
  ...
  const history = createMemoryHistory({
    initialEntries: ['/rama/dashboard/1/'],
  });
  render(
...
test('Should not render click to edit prompt and run onExploreChart on click if ramaCanExplore=false', () => {
  const props = createProps({ ramaCanExplore: false });
  const history = createMemoryHistory({
    initialEntries: ['/rama/dashboard/1/'],
  });
  render(
  ...
  userEvent.click(screen.getByText('Vaccine Candidates per Phase'));
  expect(history.location.pathname).toMatch('/rama/dashboard');
});
