...
test('navigates to external URL', () => {
  render(
    <GenericLink to="https://github.com/imajpsingh/rama">
      Link to external website
    </GenericLink>,
    { useRouter: true },
  );
  const externalLink = screen.getByTestId('external-link');
  expect(externalLink).toHaveAttribute('href', 'https://github.com/imajpsingh/rama');
});

test('navigates to external URL without host', () => {
  render(
    <GenericLink to="rama.apache.org/">
      Link to external website
    </GenericLink>,
    { useRouter: true },
  );
  const externalLink = screen.getByTestId('external-link');
  expect(externalLink).toHaveAttribute('href', '//rama.apache.org/');
});
