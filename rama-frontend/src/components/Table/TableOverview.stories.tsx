...
Common table applications in Rama:
...
you can register \`onDragOver\` and \`onDragDrop\` event handlers on the destination component. In the \`onDragDrop\` handler you can check for \`RAMA_TABLE_COLUMN\`
as the getData key as shown below.

\`\`\`
import { RAMA_TABLE_COLUMN } from 'src/components/table';

const handleDrop = (ev:Event) => {
  const json = ev.dataTransfer?.getData?.(RAMA_TABLE_COLUMN);
