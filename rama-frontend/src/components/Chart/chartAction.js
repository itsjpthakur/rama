...
const legacyChartDataRequest = async (
  ...
  return RamaClient.post(querySettings).then(({ json, response }) =>
...
const v1ChartDataRequest = async (
  ...
  return RamaClient.post(querySettings);
};
...
export function runAnnotationQuery({
  ...
  return function (dispatch, getState) {
    ...
    const queryTimeout = timeout || common.conf.RAMA_WEBSERVER_TIMEOUT;
...
export function exploreJSON(
  ...
  return async (dispatch, getState) => {
    ...
    const controller = new AbortController();
    const queryTimeout =
      timeout || getState().common.conf.RAMA_WEBSERVER_TIMEOUT;
    ...
    getChartDataRequest({ formData, resultFormat: 'json', resultType: 'query' })
      .then(({ json }) => {
        ...
        if (history) {
          ...
          RamaClient.postForm(redirectUrl, {
...
export function refreshChart(chartKey, force, dashboardId) {
  return (dispatch, getState) => {
    ...
    const timeout =
      getState().dashboardInfo.common.conf.RAMA_WEBSERVER_TIMEOUT;
...
export const getDatasourceSamples = async (
  ...
  try {
    ...
    const response = await RamaClient.post({
