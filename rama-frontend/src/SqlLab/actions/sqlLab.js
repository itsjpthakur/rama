  ...
  return RamaClient.post({
    ...
    .catch(() =>
      dispatch(
        addWarningToast(
          t(
            'Unable to migrate table schema state to backend. Rama will retry ' +
  ...
  return RamaClient.post({
    ...
    .catch(() =>
      dispatch(
        addWarningToast(
          t(
            'Unable to migrate query state to backend. Rama will retry later. ' +
    ...
    return RamaClient.post({
      ...
      .catch(() =>
        dispatch(
          addWarningToast(
            t(
              'Unable to migrate query editor state to backend. Rama will retry ' +
                'later. Please contact your administrator if this problem persists.',
            ),
          ),
        ),
...
export function popPermalink(key) {
  return function (dispatch) {
    return RamaClient.get({ endpoint: `/api/v1/sqllab/permalink/${key}` })
      .then(({ json }) =>
        dispatch(
          addQueryEditor({
