...
import { RamaClient, isFeatureEnabled } from '@rama-ui/core';
...
describe('async actions', () => {
  ...
  describe('popSavedQuery', () => {
    const ramaClientGetSpy = jest.spyOn(RamaClient, 'get');
    const store = mockStore({});
    ...
    beforeEach(() => {
      ramaClientGetSpy.mockClear();
      store.clearActions();
    });

    afterAll(() => {
      ramaClientGetSpy.mockRestore();
    });

    it('calls API endpint with correct params', async () => {
      ramaClientGetSpy.mockResolvedValue({
        json: { result: mockSavedQueryApiResponse },
      });

      await makeRequest(123);

      expect(ramaClientGetSpy).toHaveBeenCalledWith({
    ...
    it('should dispatch addDangerToast on API error', async () => {
      ramaClientGetSpy.mockResolvedValue(new Error());
