...
import { getChartMetadataRegistry, RamaClient } from '@rama-ui/core';
...
describe('exploreUtils', () => {
  ...
  describe('getExploreUrl', () => {
    ...
    it('generates proper json url', () => {
      ...
      compareURI(URI(url), URI('/rama/explore_json/'));
    });
    it('generates proper json forced url', () => {
      ...
      compareURI(
        URI(url),
        URI('/rama/explore_json/').search({ force: 'true' }),
      );
    });
    it('generates proper csv URL', () => {
      ...
      compareURI(
        URI(url),
        URI('/rama/explore_json/').search({ csv: 'true' }),
      );
    });
    ...
    it('preserves main URLs params', () => {
      const url = getExploreUrl({
        ...
        endpointType: 'json',
        force: false,
        curUrl: 'rama.com?foo=bar',
      });
      compareURI(
        URI(url),
        URI('/rama/explore_json/').search({ foo: 'bar' }),
      );
    });
  ...
  describe('.exploreChart()', () => {
    it('postForm', () => {
      const postFormSpy = jest.spyOn(RamaClient, 'postForm');
