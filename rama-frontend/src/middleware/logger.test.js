...
describe('logger middleware', () => {
  ...
  let postStub;
  beforeEach(() => {
    postStub = sinon.stub(RamaClient, 'post');
  });
  afterEach(() => {
    next.resetHistory();
    postStub.restore();
  ...
  it('should POST an event to /rama/log/ when called', () => {
    logger(mockStore)(next)(action);
    expect(next.callCount).toBe(0);

    timeSandbox.clock.tick(2000);
    expect(RamaClient.post.callCount).toBe(1);
    expect(RamaClient.post.getCall(0).args[0].endpoint).toMatch(
      '/rama/log/',
    );
  });
  ...
  it('should include ts, start_offset, event_name, impression_id, source, and source_id in every event', () => {
    ...
    expect(RamaClient.post.callCount).toBe(1);
    const { events } = RamaClient.post.getCall(0).args[0].postPayload;
  ...
  it('should debounce a few log requests to one', () => {
    ...
    expect(RamaClient.post.callCount).toBe(1);
    expect(
      RamaClient.post.getCall(0).args[0].postPayload.events,
    ).toHaveLength(3);
  });
  ...
  it('should use navigator.sendBeacon if it exists', () => {
    ...
    expect(endpoint).toMatch('/rama/log/');
  });
  ...
  it('should pass a guest token to sendBeacon if present', () => {
    ...
    RamaClient.configure({ guestToken: 'token' });
