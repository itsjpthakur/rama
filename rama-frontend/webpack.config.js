...
// output dir
const BUILD_DIR = path.resolve(__dirname, '../rama/static/assets');
...
if (isDevMode) {
  // A Rama webpage normally includes two JS bundles in dev, `theme.ts` and
...
const config = {
  ...
  ignoreWarnings: [
    ...
    {
      message: /Can't resolve.*rama_text/,
    },
  ],
  performance: {
  ...
  resolve: {
    // resolve modules from `/rama_frontend/node_modules` and `/rama_frontend`
  ...
  module: {
    rules: [
      ...
      },
      {
        ...
        exclude: [/rama-ui.*\/node_modules\//, /\.test.jsx?$/],
        include: [
...
Object.entries(packageConfig.dependencies).forEach(([pkg, relativeDir]) => {
  ...
  if (/^@rama-ui/.test(pkg) && fs.existsSync(srcPath)) {
    console.log(`[Rama Plugin] Use symlink source for ${pkg} @ ${dir}`);
...
if (process.env.BUNDLE_ANALYZER) {
  ...
  config.plugins.push(
    ...
    // you'll find it at rama/static/stats/statistics.html
