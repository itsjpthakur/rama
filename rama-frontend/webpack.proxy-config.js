...
const { ramaPort = 8088, rama: ramaUrl = null } = parsedEnvArg();
const backend = (ramaUrl || `http://localhost:${ramaPort}`).replace(
  '//+$/',
  '',
...
function toDevHTML(originalHtml) {
  ...
  if (manifest) {
    ...
    // in rama/templates/rama/partials/asset_bundle.html
    html = html.replace(
