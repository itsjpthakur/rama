...
import { ensureIsArray, styled } from '@rama-ui/core';
...
const StyledDiv = styled.div`
  ${({ theme }) => `
    .rama-legacy-chart-horizon {
      overflow: auto;
      position: relative;
    }

    .rama-legacy-chart-horizon .horizon-row {
    ...
    .rama-legacy-chart-horizon .horizon-row span.title {
      position: absolute;
...
class HorizonChart extends PureComponent {
  render() {
    ...
    return (
      <StyledDiv>
        <div
          className={`rama-legacy-chart-horizon ${className}`}
          style={{ height }}
        >
          {data.map(row => (
            <HorizonRow
