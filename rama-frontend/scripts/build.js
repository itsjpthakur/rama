...
const META_PACKAGES = new Set(['demo', 'generator-rama']);
...
function getPackages(packagePattern, tsOnly = false) {
  ...
  if (pattern === '*' && !tsOnly) {
    return `@rama-ui/!(${[...META_PACKAGES].join('|')})`;
  }
  if (!pattern.includes('*')) {
    pattern = `*${pattern}`;
  }
  const packages = [
    ...new Set(
      fastGlob
        .sync([
          `./node_modules/@rama-ui/${pattern}/src/**/*.${
...
if (shouldCleanup) {
  ...
  const dirtyModules = 'node_modules/@types/react,node_modules/@rama-ui';
