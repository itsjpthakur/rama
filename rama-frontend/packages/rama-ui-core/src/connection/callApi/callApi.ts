...
export default async function callApi({
  ...
  if (
    ...
    let ramaCache: Cache | null = null;
    try {
      ramaCache = await caches.open(CACHE_KEY);
      const cachedResponse = await ramaCache.match(url);
      if (cachedResponse) {
      ...
      // If rama is in an iframe and third-party cookies are disabled, caches.open throws
    }

    const response = await fetchWithRetry(url, request);

    if (ramaCache && response.status === HTTP_STATUS_NOT_MODIFIED) {
      const cachedFullResponse = await ramaCache.match(url);
    ...
    if (
      ...
      ramaCache.delete(url);
      ramaCache.put(url, response.clone());
    }
