...
import {
  RamaApiError,
  RamaApiErrorPayload,
  RamaApiMultiErrorsPayload,
} from './types';

export type ErrorInput = string | Error | Response | RamaApiErrorPayload;

/**
 * Handle API request errors, convert to consistent Rama API error.
 * @param error the catched error from RamaClient.request(...)
 */
export default async function handleError(error: ErrorInput): Promise<never> {
  // already a Rama error
  if (error instanceof RamaApiError) {
    throw error;
  }
  // string is the error message itself
  if (typeof error === 'string') {
    throw new RamaApiError({ message: error });
  }
  // JS errors, normally happens before request was sent
  if (error instanceof Error) {
    throw new RamaApiError({
