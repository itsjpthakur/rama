...
interface RamaApiFactoryOptions extends Omit<RequestBase, 'url'> {
  /**
...
>(
  ...
  async function request(
    payload: Payload,
    { client = RamaClient }: RamaApiRequestOptions = {
      client: RamaClient,
    },
  ): Promise<Result> {
    try {
      ...
      if (responseType === 'text') {
        ...
        if (result && typeof result === 'object' && 'error' in result) {
          return handleError(result as RamaApiErrorPayload);
        }
      }
