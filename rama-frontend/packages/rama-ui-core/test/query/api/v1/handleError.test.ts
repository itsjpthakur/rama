...
async function testHandleError(
...
): Promise<RamaApiError> {
  try {
    await handleError(inputError);
  } catch (error) {
    const typedError = error as RamaApiError;
    expect(typedError).toBeInstanceOf(RamaApiError);
  ...
  return new RamaApiError({ message: 'Where is the error?' });
}

describe('handleError()', () => {
  it('should throw error directly', async () => {
    expect.assertions(3);
    const input = new RamaApiError({ message: 'timeout' });
  ...
  it('should handle response of single error', async () => {
    ...
    const mockResponse = new Response(
      '{ "error": "BAD BAD", "link": "https://github.com/imajpsingh/rama" }',
      {
        status: 403,
    ...
    await testHandleError(mockResponse, {
      message: 'BAD BAD',
      extra: { link: 'https://github.com/imajpsingh/rama' },
    });
  });
  ...
  it('should handle single error object', async () => {
    ...
    const mockError = {
      error: {
        ...
        error_type: RamaApiErrorType.FrontendTimeoutError,
      },
    };
