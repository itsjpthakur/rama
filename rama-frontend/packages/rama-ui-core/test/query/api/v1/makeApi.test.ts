...
import { JsonValue, RamaClientClass } from '@rama-ui/core';
import { makeApi, RamaApiError } from '../../../../src/query';
...
describe('makeApi()', () => {
  ...
  it('should allow custom client', async () => {
    ...
    const client = new RamaClientClass({ baseUrl: 'http://foo/' });
  ...
  it('should handle errors', async () => {
    ...
    try {
      ...
      expect((error as RamaApiError).message).toContain('Invalid payload');
    }
  });
  ...
  it('should handle error on 200 response', async () => {
    ...
    try {
      ...
      expect((error as RamaApiError).message).toContain('not ok');
    }
  });
