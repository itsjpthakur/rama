...
import { RamaClientClass, ClientConfig, CallApi } from '@rama-ui/core';
import { LOGIN_GLOB } from './fixtures/constants';

describe('RamaClientClass', () => {
  beforeEach(() => {
    fetchMock.reset();
  ...
  describe('new RamaClientClass()', () => {
    it('fallback protocol to https when setting only host', () => {
      const client = new RamaClientClass({ host: 'TEST-HOST' });
      expect(client.baseUrl).toEqual('https://test-host');
    });
  });

  describe('.getUrl()', () => {
    let client = new RamaClientClass();

    beforeEach(() => {
      client = new RamaClientClass({
        protocol: 'https:',
        host: 'CONFIG_HOST',
      });
    });
    ...
    it('does not throw if url, endpoint, and host are all empty', () => {
      client = new RamaClientClass({ protocol: 'https:', host: '' });
  ...
  describe('.init()', () => {
    ...
    it('calls api/v1/security/csrf_token/ when init() is called if no CSRF token is passed', async () => {
      expect.assertions(1);
      await new RamaClientClass().init();
    ...
    it('does NOT call api/v1/security/csrf_token/ when init() is called if a CSRF token is passed', async () => {
      expect.assertions(1);
      await new RamaClientClass({ csrfToken: 'abc' }).init();
    ...
    it('calls api/v1/security/csrf_token/ when init(force=true) is called even if a CSRF token is passed', async () => {
      ...
      const client = new RamaClientClass({ csrfToken: initialToken });
  ...
  describe('.isAuthenticated()', () => {
    ...
    it('returns true if there is a token and false if not', async () => {
      expect.assertions(2);
      const client = new RamaClientClass({});
    ...
    it('returns true if a token is passed at configuration', () => {
      expect.assertions(2);
      const clientWithoutToken = new RamaClientClass({
        csrfToken: undefined,
      });
      const clientWithToken = new RamaClientClass({ csrfToken: 'token' });
  ...
  describe('requests', () => {
    ...
    it('checks for authentication before every get and post request', async () => {
      ...
      const authSpy = jest.spyOn(RamaClientClass.prototype, 'ensureAuth');
      const client = new RamaClientClass({ protocol, host });
    ...
    it('sets protocol, host, headers, mode, and credentials from config', async () => {
      ...
      const client = new RamaClientClass(clientConfig);
      await client.init();
      await client.get({ url: mockGetUrl });
  ...
  describe('when unauthorized', () => {
    ...
    beforeEach(() => {
      ...
      authSpy = jest
        .spyOn(RamaClientClass.prototype, 'ensureAuth')
        .mockImplementation();
    ...
    it('accepts an unauthorizedHandler to override redirect behavior', async () => {
      ...
      const client = new RamaClientClass({ unauthorizedHandler });
  ...
  describe('.postForm()', () => {
    ...
    beforeEach(async () => {
      ...
      client = new RamaClientClass({ protocol, host });
      authSpy = jest.spyOn(RamaClientClass.prototype, 'ensureAuth');
      await client.init();
      appendChild = jest.fn();
      removeChild = jest.fn();
      submit = jest.fn();
    ...
    it('makes postForm request with guest token', async () => {
      client = new RamaClientClass({ protocol, host, guestToken });
      await client.init();
