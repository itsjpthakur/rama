...
import { CallApi, JsonObject } from '@rama-ui/core';
...
describe('callApi()', () => {
  ...
  describe('caching', () => {
    ...
    it('caches requests with ETags', async () => {
      ...
      expect(calls).toHaveLength(1);
      const ramaCache = await caches.open(constants.CACHE_KEY);
      const cachedResponse = await ramaCache.match(mockCacheUrl);
