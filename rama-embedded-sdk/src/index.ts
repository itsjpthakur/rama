...
import { Switchboard } from '@rama-ui/switchboard';
...
export type EmbedDashboardParams = {
  /** The id provided by the embed configuration UI in Rama */
  id: string
  /** The domain where Rama can be located, with protocol, such as: https://rama.example.com */
  ramaDomain: string
...
/**
 * Embeds a Rama dashboard into the page using an iframe.
 */
export async function embedDashboard({
  ...
  function log(...info: unknown[]) {
    if (debug) {
      console.debug(`[rama-embedded-sdk][dashboard ${id}]`, ...info);
    }
  }

  log('embedding');

  if (ramaDomain.endsWith("/")) {
    ramaDomain = ramaDomain.slice(0, -1);
  }
  ...
  async function mountIframe(): Promise<Switchboard> {
    return new Promise(resolve => {
      ...
      iframe.addEventListener('load', () => {
        ...
        resolve(new Switchboard({ port: ourPort, name: 'rama-embedded-sdk', debug }));
      });
      iframe.src = `${ramaDomain}/embedded/${id}${urlParamsString}`;
