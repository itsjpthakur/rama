...
[![Version](https://img.shields.io/npm/v/%40rama-ui%2Fembedded-sdk?style=flat)](https://www.npmjs.com/package/@rama-ui/embedded-sdk)
[![Libraries.io](https://img.shields.io/librariesio/release/npm/%40rama-ui%2Fembedded-sdk?style=flat)](https://libraries.io/npm/@rama-ui%2Fembedded-sdk)

# Rama Embedded SDK

The Embedded SDK allows you to embed dashboards from Rama into your own app,
using your app's authentication.

Embedding is done by inserting an iframe, containing a Rama page, into the host application.

## Prerequisites

* Activate the feature flag `EMBEDDED_RAMA`
...
```sh
npm install --save @rama-ui/embedded-sdk
```

```js
import { embedDashboard } from "@rama-ui/embedded-sdk";

embedDashboard({
  id: "abc123", // given by the Rama embedding UI
  ramaDomain: "https://rama.example.com",
  mountPoint: document.getElementById("my-rama-container"), // any html element that can contain an iframe
...
You can also load the Embedded SDK from a CDN. The SDK will be available as `ramaEmbeddedSdk` globally:

```html
<script src="https://unpkg.com/@rama-ui/embedded-sdk"></script>

<script>
  ramaEmbeddedSdk.embedDashboard({
...
Embedded resources use a special auth token called a Guest Token to grant Rama access to your users,
without requiring your users to log in to Rama directly. Your backend must create a Guest Token
by requesting Rama's `POST /security/guest_token` endpoint, and pass that guest token to your frontend.
...
Within your app, using the Guest Token will then allow authentication to your Rama instance via creating an Anonymous user object.  This guest anonymous user will default to the public role as per this setting `GUEST_ROLE_NAME = "Public"`.
