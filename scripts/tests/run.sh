...
function reset_db() {
  ...
  echo --------------------
  docker compose stop rama-tests-worker rama || true
...
EOF
"
  docker exec -i rama_db bash -c "${RESET_DB_CMD}"
  docker compose start rama-tests-worker rama
}
...
export RAMA__SQLALCHEMY_DATABASE_URI=${RAMA__SQLALCHEMY_DATABASE_URI:-postgresql+psycopg2://"${DB_USER}":"${DB_PASSWORD}"@localhost/"${DB_NAME}"}
...
while (( "$#" )); do
  case "$1" in
    --help)
      echo Switches:
      echo --no-init : Will not, reset the test DB, rama init and load examples
      echo --no-reset-db: Will not reset the test DB
      echo --no-tests: Will not run any test, by default reset the DB, rama init and load_examples
...
echo ------------------------------------
echo DB_URI="${RAMA__SQLALCHEMY_DATABASE_URI}"
