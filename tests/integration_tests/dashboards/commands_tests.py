...
from rama.commands.dashboard.delete import DeleteEmbeddedDashboardCommand
...
from rama.commands.dashboard.export import (
    append_charts,
    ExportDashboardsCommand,
    get_default_position,
)
from rama.commands.dashboard.fave import AddFavoriteDashboardCommand
from rama.commands.dashboard.importers import v0, v1
...
from rama.daos.dashboard import DashboardDAO
...
class TestExportDashboardsCommand(RamaTestCase):
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @patch("rama.security.manager.g")
    @patch("rama.views.base.g")
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @patch("rama.commands.dashboard.export.suffix")
...
class TestImportDashboardsCommand(RamaTestCase):
...
class TestCopyDashboardCommand(RamaTestCase):
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    ...
    def test_copy_dashboard_command_no_access(self):
        ...
        with self.client.application.test_request_context():
            ...
            with override_user(security_manager.find_user("gamma")):
                with patch(
                    "rama.commands.dashboard.copy.is_feature_enabled",
                    return_value=True,
                ):
                    command = CopyDashboardCommand(example_dashboard, copy_data)
...
class TestDeleteEmbeddedDashboardCommand(RamaTestCase):
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
...
class TestFavoriteDashboardCommand(RamaTestCase):
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    def test_fave_unfave_dashboard_command(self):
        ...
        with self.client.application.test_request_context():
            ...
            with override_user(security_manager.find_user("admin")):
                with patch(
                    "rama.daos.dashboard.DashboardDAO.get_by_id_or_slug",
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @patch("rama.models.dashboard.Dashboard.get")
