...
from rama import db, security_manager  # noqa: F401
from rama.models.dashboard import Dashboard
from rama.models.core import FavStar, FavStarClassName
from rama.reports.models import ReportSchedule, ReportScheduleType
from rama.models.slice import Slice
from rama.tags.models import Tag, TaggedObject, TagType, ObjectType
from rama.utils.core import backend, override_user
from rama.utils.screenshots import ScreenshotCachePayload
...
class TestDashboardApi(ApiOwnersTestCaseMixin, InsertChartMixin, RamaTestCase):
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @patch("rama.dashboards.schemas.security_manager.has_guest_access")
    @patch("rama.dashboards.schemas.security_manager.is_guest_user")
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @patch("rama.utils.log.logger")
    @patch("rama.daos.dashboard.DashboardDAO.get_datasets_for_dashboard")
        ...
        expected_results = [
            {
                ...
                "url": "/rama/dashboard/create_slug1/",
            },
            {
                "dashboard_title": "create_title0",
                "url": "/rama/dashboard/create_slug0/",
            },
        ]
    ...
    def test_delete_dashboard_not_owned(self):
        ...
        user_alpha1 = self.create_user(
            "alpha1", "password", "Alpha", email="alpha1@rama.org"
        )
        user_alpha2 = self.create_user(
            "alpha2", "password", "Alpha", email="alpha2@rama.org"
        )
        existing_slice = (
    ...
    @with_feature_flags(THUMBNAILS=True, ENABLE_DASHBOARD_SCREENSHOT_ENDPOINTS=True)
    @pytest.mark.usefixtures("create_dashboard_with_tag")
    @patch("rama.dashboards.api.cache_dashboard_screenshot")
    @patch("rama.dashboards.api.DashboardScreenshot.get_from_cache_key")
    ...
    @pytest.mark.usefixtures("create_dashboard_with_tag")
    @patch("rama.dashboards.api.cache_dashboard_screenshot")
    @patch("rama.dashboards.api.build_pdf_from_screenshots")
    @patch("rama.dashboards.api.DashboardScreenshot.get_from_cache_key")
