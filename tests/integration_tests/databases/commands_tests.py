...
from rama import db, event_logger, security_manager  # noqa: F401
from rama.commands.database.create import CreateDatabaseCommand
...
from rama.commands.database.export import ExportDatabasesCommand
from rama.commands.database.importers.v1 import ImportDatabasesCommand
from rama.commands.database.tables import TablesDatabaseCommand
from rama.commands.database.test_connection import TestConnectionDatabaseCommand
from rama.commands.database.validate import ValidateDatabaseParametersCommand
from rama.commands.exceptions import CommandInvalidError
from rama.commands.importers.exceptions import IncorrectVersionError
from rama.connectors.sqla.models import SqlaTable
from rama.databases.schemas import DatabaseTestConnectionSchema  # noqa: F401
...
from rama.utils.core import backend
...
class TestCreateDatabaseCommand(RamaTestCase):
    @patch("rama.commands.database.test_connection.event_logger.log_with_context")
    @patch("rama.utils.core.g")
...
class TestExportDatabasesCommand(RamaTestCase):
    @skip("Flaky")
    @patch("rama.security.manager.g")
...
class TestImportDatabasesCommand(RamaTestCase):
    @patch("rama.security.manager.g")
    @patch("rama.commands.database.importers.v1.utils.add_permissions")
    ...
    @patch("rama.databases.schemas.is_feature_enabled")
    @patch("rama.commands.database.importers.v1.utils.add_permissions")
    ...
    @patch("rama.commands.database.importers.v1.import_dataset")
    @patch("rama.commands.database.importers.v1.utils.add_permissions")
...
class TestTestConnectionDatabaseCommand(RamaTestCase):
    @patch("rama.daos.database.Database._get_sqla_engine")
    @patch("rama.commands.database.test_connection.event_logger.log_with_context")
    @patch("rama.utils.core.g")
    ...
    def test_connection_do_ping_exception(
        ...
        with pytest.raises(RamaErrorsException) as excinfo:
            command_without_db_name.run()
        assert (
    ...
    def test_connection_do_ping_timeout(
        ...
        database = get_example_database()
        mock_g.user = security_manager.find_user("admin")
        mock_func_timeout.side_effect = RamaTimeoutException(
            error_type=RamaErrorType.BACKEND_TIMEOUT_ERROR,
            message="ERROR",
        ...
        with pytest.raises(RamaTimeoutException) as excinfo:
            command_without_db_name.run()
        assert excinfo.value.status == 408
        assert (
            excinfo.value.error.error_type
            == RamaErrorType.CONNECTION_DATABASE_TIMEOUT
        )

    @patch("rama.daos.database.Database._get_sqla_engine")
    @patch("rama.commands.database.test_connection.event_logger.log_with_context")
    @patch("rama.utils.core.g")
    def test_connection_rama_security_connection(
        self, mock_g, mock_event_logger, mock_get_sqla_engine
    ):
        ...
        database = get_example_database()
        mock_g.user = security_manager.find_user("admin")
        mock_get_sqla_engine.side_effect = RamaSecurityException(
            RamaError(error_type=500, message="test", level="info")
        )
        db_uri = database.sqlalchemy_uri_decrypted
        json_payload = {"sqlalchemy_uri": db_uri}
        command_without_db_name = TestConnectionDatabaseCommand(json_payload)
    ...
    def test_connection_db_api_exc(
        ...
        with pytest.raises(RamaErrorsException) as excinfo:  # noqa: PT012
            command_without_db_name.run()
            assert str(excinfo.value) == (
...
@patch("rama.db_engine_specs.base.is_hostname_valid")
@patch("rama.db_engine_specs.base.is_port_open")
@patch("rama.commands.database.validate.DatabaseDAO")
...
def test_validate_partial_invalid_hostname(is_hostname_valid, app_context):
    ...
    assert excinfo.value.errors == [
        ...
        RamaError(
            ...
            error_type=RamaErrorType.CONNECTION_INVALID_HOSTNAME_ERROR,
            level=ErrorLevel.ERROR,
            extra={
                "invalid": ["host"],
                "issue_codes": [
...
class TestTablesDatabaseCommand(RamaTestCase):
    @patch("rama.daos.database.DatabaseDAO.find_by_id")
    ...
    @patch("rama.daos.database.DatabaseDAO.find_by_id")
    @patch("rama.security.manager.RamaSecurityManager.can_access_database")
    @patch("rama.utils.core.g")
    def test_database_tables_rama_exception(
        ...
        mock_can_access_database.side_effect = RamaException("Test Error")
        mock_g.user = security_manager.find_user("admin")

        command = TablesDatabaseCommand(database.id, None, "main", False)
        with pytest.raises(RamaException) as excinfo:  # noqa: PT012
            command.run()
