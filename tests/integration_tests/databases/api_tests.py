...
from rama import db, security_manager
from rama.connectors.sqla.models import SqlaTable
from rama.databases.ssh_tunnel.models import SSHTunnel
from rama.databases.utils import make_url_safe  # noqa: F401
from rama.db_engine_specs.mysql import MySQLEngineSpec
from rama.db_engine_specs.postgres import PostgresEngineSpec
from rama.db_engine_specs.redshift import RedshiftEngineSpec
from rama.db_engine_specs.bigquery import BigQueryEngineSpec
from rama.db_engine_specs.gsheets import GSheetsEngineSpec
from rama.db_engine_specs.hana import HanaEngineSpec
from rama.errors import RamaError
from rama.models.core import Database, ConfigurationMethod
from rama.reports.models import ReportSchedule, ReportScheduleType
from rama.utils.database import get_example_database, get_main_database
...
class TestDatabaseApi(RamaTestCase):
    def insert_database(
        self,
        database_name: str,
        sqlalchemy_uri: str,
    ...
    @mock.patch(
        "rama.commands.database.test_connection.TestConnectionDatabaseCommand.run",
    )
    @mock.patch("rama.commands.database.create.is_feature_enabled")
    @mock.patch("rama.models.core.Database.get_all_catalog_names")
    @mock.patch("rama.models.core.Database.get_all_schema_names")
    ...
    @mock.patch("rama.commands.database.create.is_feature_enabled")
    @mock.patch("rama.commands.database.update.is_feature_enabled")
    @mock.patch("rama.models.core.Database.get_all_catalog_names")
    @mock.patch("rama.models.core.Database.get_all_schema_names")
    ...
    @mock.patch("rama.models.core.Database.get_all_catalog_names")
    @mock.patch("rama.models.core.Database.get_all_schema_names")
    @mock.patch("rama.extensions.db.session.rollback")
    ...
    def test_create_database_conn_fail(self):
        ...
        response = self.client.post(uri, json=database_data)
        response_data = json.loads(response.data.decode("utf-8"))
        rama_error_mysql = RamaError(
            message='Either the username "rama" or the password is incorrect.',
        ...
        rama_error_postgres = RamaError(
            message='The password provided for username "rama" is incorrect.',
        ...
        expected_response_mysql = {"errors": [dataclasses.asdict(rama_error_mysql)]}
        expected_response_postgres = {
            "errors": [dataclasses.asdict(rama_error_postgres)]
        }
    ...
    @patch("rama.utils.log.logger")
    @mock.patch("rama.security.manager.RamaSecurityManager.can_access_database")
    @mock.patch("rama.models.core.Database.get_all_table_names_in_schema")
            ...
            "errors": [
                {
                    ...
                    "extra": {
                        "issue_codes": [
                            {
                                "code": 1010,
                                "message": "Issue 1010 - Rama encountered an error while running a command.",  # noqa: E501
                            }
                        ]
                    },
                }
    ...
    def test_test_connection_unsafe_uri(self):
        ...
        app.config["PREVENT_UNSAFE_DB_CONNECTIONS"] = True
        data = {
            "sqlalchemy_uri": "sqlite:///home/rama/unsafe.db",
    ...
    @mock.patch(
        "rama.commands.database.test_connection.DatabaseDAO.build_db_for_connection_test",
    )
    @mock.patch(
        "rama.commands.database.test_connection.event_logger",
    )
    def test_test_connection_failed_invalid_hostname(
        ...
        assert rv.headers["Content-Type"] == "application/json; charset=utf-8"
        response = json.loads(rv.data.decode("utf-8"))
        expected_response = {"errors": [dataclasses.asdict(rama_error)]}
        assert response == expected_response
    ...
    @mock.patch("rama.commands.database.importers.v1.utils.add_permissions")
                            ...
                            "issue_codes": [
                                {
                                    "code": 1010,
                                    "message": (
                                        "Issue 1010 - Rama encountered an "
    ...
    @mock.patch("rama.databases.schemas.is_feature_enabled")
    @mock.patch("rama.commands.database.importers.v1.utils.add_permissions")
    ...
    @mock.patch(
        "rama.db_engine_specs.base.BaseEngineSpec.get_function_names",
    )
    ...
    @mock.patch("rama.databases.api.get_available_engine_specs")
    @mock.patch("rama.databases.api.app")
    ...
    @mock.patch("rama.db_engine_specs.base.is_hostname_valid")
    @mock.patch("rama.db_engine_specs.base.is_port_open")
    @mock.patch("rama.databases.api.ValidateDatabaseParametersCommand")
    ...
    @mock.patch.dict(
        "rama.config.SQL_VALIDATORS_BY_ENGINE",
        SQL_VALIDATORS_BY_ENGINE,
        clear=True,
    )
    ...
    @mock.patch("rama.commands.database.validate_sql.get_validator_by_name")
    @mock.patch.dict(
        "rama.config.SQL_VALIDATORS_BY_ENGINE",
        PRESTO_SQL_VALIDATORS_BY_ENGINE,
        clear=True,
    ...
    def test_get_databases_with_extra_filters(self):
        ...
        with patch.dict(
            "rama.views.filters.current_app.config",
