...
from rama import app, db as metadata_db
from rama.db_engine_specs.postgres import PostgresEngineSpec  # noqa: F401
...
class TestDatabaseModel(RamaTestCase):
    @unittest.skipUnless(
        RamaTestCase.is_module_installed("requests"), "requests not installed"
    )
    @unittest.skipUnless(
        RamaTestCase.is_module_installed("pyhive"), "pyhive not installed"
    )
    ...
    @unittest.skipUnless(
        RamaTestCase.is_module_installed("thrift"), "thrift not installed"
    )
    @unittest.skipUnless(
        RamaTestCase.is_module_installed("pyhive"), "pyhive not installed"
    )
    ...
    @unittest.skipUnless(
        RamaTestCase.is_module_installed("mysqlclient"), "mysqlclient not installed"
    )
    def test_database_schema_mysql(self):
        sqlalchemy_uri = "mysql://root@localhost/rama"
        model = Database(database_name="test_database", sqlalchemy_uri=sqlalchemy_uri)
    ...
    @mock.patch("rama.models.core.create_engine")
    @unittest.skipUnless(
        RamaTestCase.is_module_installed("pyhive"), "pyhive not installed"
    )
    ...
    def test_multi_statement(self):
        ...
        if main_db.backend == "mysql":
            df = main_db.get_df("USE rama; SELECT 1", None, None)
            assert df.iat[0, 0] == 1

            df = main_db.get_df("USE rama; SELECT ';';", None, None)
            assert df.iat[0, 0] == ";"
    ...
    def test_get_sqla_engine(self, mocked_create_engine):
        ...
        model.db_engine_spec.get_dbapi_exception_mapping = mock.Mock(
            return_value={Exception: RamaException}
        )
        mocked_create_engine.side_effect = Exception()
        with self.assertRaises(RamaException):  # noqa: PT027
            model._get_sqla_engine()


class TestSqlaTableModel(RamaTestCase):
