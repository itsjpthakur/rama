...
from rama.connectors.sqla.models import SqlaTable  # noqa: F401
from rama.db_engine_specs import BaseEngineSpec
from rama.db_engine_specs.hive import HiveEngineSpec
from rama.db_engine_specs.presto import PrestoEngineSpec
from rama.errors import ErrorLevel, RamaError, RamaErrorType
from rama.exceptions import RamaErrorException
...
from rama.sql_lab import (
    cancel_query,
    execute_sql_statements,
    apply_limit_if_exists,
)
from rama.sql_parse import CtasMethod
from rama.utils.core import backend
from rama.utils import json
from rama.utils.json import datetime_to_epoch  # noqa: F401
...
class TestSqlLab(RamaTestCase):
    ...
    def test_sql_json(self):
        ...
        if backend() == "presto":
            assert (
                ...
                == RamaErrorType.TABLE_DOES_NOT_EXIST_ERROR
            )
                ...
                "errors": [
                    {
                        ...
                        "error_type": RamaErrorType.DML_NOT_ALLOWED_ERROR,
    ...
    def test_sql_json_cta_dynamic_db(self, ctas_method):
        ...
        with mock.patch(
            "rama.sqllab.sqllab_execution_context.get_cta_schema_name",
    ...
    def test_ps_conversion_no_dict(self):
        ...
        data = [["a", 4, 4.0]]
        results = RamaResultSet(data, cols, BaseEngineSpec)
    ...
    @mock.patch("rama.sql_lab.db")
    @mock.patch("rama.sql_lab.get_query")
    @mock.patch("rama.sql_lab.execute_sql_statement")
    ...
    @mock.patch("rama.sql_lab.results_backend", None)
    @mock.patch("rama.sql_lab.get_query")
    @mock.patch("rama.sql_lab.execute_sql_statement")
    def test_execute_sql_statements_no_results_backend(
        ...
        with pytest.raises(RamaErrorException) as excinfo:
            execute_sql_statements(
                query_id=1,
                rendered_query=sql,
                return_results=True,
    ...
    def test_execute_sql_statements_ctas(
        ...
        assert excinfo.value.error == RamaError(
            ...
            error_type=RamaErrorType.INVALID_CTAS_QUERY_ERROR,
            level=ErrorLevel.ERROR,
            extra={
                "issue_codes": [
                    {
        ...
        assert excinfo.value.error == RamaError(
            ...
            error_type=RamaErrorType.INVALID_CVAS_QUERY_ERROR,
            level=ErrorLevel.ERROR,
            extra={
                "issue_codes": [
                    {
                ...
                "errors": [
                    {
                        ...
                        "error_type": RamaErrorType.SQLLAB_TIMEOUT_ERROR,
                        "level": ErrorLevel.ERROR,
                        "extra": {
                            "issue_codes": [
                                {
