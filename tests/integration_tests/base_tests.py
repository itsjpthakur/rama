...
from rama.utils.core import get_example_default_schema, shortid
from rama.utils import json
from rama.utils.database import get_example_database
from rama.views.base_api import BaseRamaModelRestApi
...
def post_assert_metric(
    ...
    :param client: test client for rama api requests
    ...
    """
    with patch.object(
        BaseRamaModelRestApi, "incr_stats", return_value=None
    ) as mock_method:
...
class RamaTestCase(TestCase):
    ...
    def get_birth_names_dataset() -> SqlaTable:
        return RamaTestCase.get_table(name="birth_names")
    ...
    def get_table(
        ...
        return (
            ...
            .filter_by(
                ...
                or RamaTestCase.get_database_by_name("examples").id,
                schema=schema,
                table_name=name,
            )
            .one()
    ...
    def run_sql(
        ...
        if username:
            self.logout()
            self.login(username)
        dbid = RamaTestCase.get_database_by_name(database_name).id
        json_payload = {
            "database_id": dbid,
            "sql": sql,
            "client_id": client_id,
