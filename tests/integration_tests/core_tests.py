...
import rama.utils.database
import rama.views.utils
from rama import dataframe, db, security_manager, sql_lab
from rama.commands.chart.data.get_data_command import ChartDataCommand
from rama.commands.chart.exceptions import ChartDataQueryFailedError
...
from rama.db_engine_specs.mssql import MssqlEngineSpec
...
from rama.result_set import RamaResultSet
...
from .base_tests import RamaTestCase
...
class TestCore(RamaTestCase):
    ...
    def test_dashboard_endpoint(self):
        ...
        resp = self.client.get("/rama/dashboard/-1/")
        assert resp.status_code == 404

    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    def test_slice_endpoint(self):
        self.login(ADMIN_USERNAME)
        resp = self.client.get("/rama/slice/-1/")
        assert resp.status_code == 404
    ...
    def test_custom_password_store(self):
        database = rama.utils.database.get_example_database()
    ...
    def test_warm_up_cache_error(self) -> None:
        ...
        with mock.patch.object(
            ...
            assert self.get_json_resp(f"/rama/warm_up_cache?slice_id={slc.id}") == [
                {
                    "slice_id": slc.id,
    ...
    def test_templated_sql_json(self):
        if rama.utils.database.get_example_database().backend == "presto":
    ...
    def test_fetch_datasource_metadata(self):
        self.login(ADMIN_USERNAME)
        url = "/rama/fetch_datasource_metadata?" "datasourceKey=1__table"
    ...
    def test_dataframe_timezone(self):
        ...
        results = RamaResultSet(list(data), [["data"]], BaseEngineSpec)
    ...
    def test_explore_json_data_invalid_cache_key(self):
        ...
        rv = self.client.get(f"/rama/explore_json/data/{cache_key}")
        data = json.loads(rv.data.decode("utf-8"))
    ...
    def test_results_default_deserialization(self):
        ...
        results = RamaResultSet(data, cursor_descr, db_engine_spec)
        query = {
            "database_id": 1,
        ...
        deserialized_payload = rama.views.utils._deserialize_results_payload(
    ...
    def test_explore_database_id(self):
        database = rama.utils.database.get_example_database()
        explore_database = rama.utils.database.get_example_database()
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @mock.patch("rama.models.core.DB_CONNECTION_MUTATOR")
    def test_explore_injected_exceptions(self, mock_db_connection_mutator):
        ...
        exception = RamaException("Error message")
    ...
    @pytest.mark.usefixtures("load_energy_table_with_slice")
    @mock.patch("rama.commands.explore.form_data.create.CreateFormDataCommand.run")
    def test_explore_redirect(self, mock_command: mock.Mock):
        ...
        rv = self.client.get(
            f"/rama/explore/?form_data={quote(json.dumps(form_data))}"
        )
    ...
    def test_has_table(self):
        if backend() in ("sqlite", "mysql"):
            return
        example_db = rama.utils.database.get_example_database()
        assert example_db.has_table(Table("birth_names", "public")) is True

    @mock.patch("rama.views.core.request")
    @mock.patch(
        "rama.commands.dashboard.permalink.get.GetDashboardPermalinkCommand.run"
    )
    def test_dashboard_permalink(self, get_dashboard_permalink_mock, request_mock):
        ...
        self.login(ADMIN_USERNAME)
        resp = self.client.get("rama/dashboard/p/123/")

        expected_url = "/rama/dashboard/1?permalink_key=123&standalone=3"
