...
from rama.views.base_api import BaseRamaModelRestApi, requires_json  # noqa: F401
...
class Model1Api(BaseRamaModelRestApi):
...
class TestOpenApiSpec(RamaTestCase):
...
class TestBaseModelRestApi(RamaTestCase):
...
class ApiOwnersTestCaseMixin:
    ...
    @patch(
        "rama.security.RamaSecurityManager.get_exclude_users_from_lists",
        return_value=["gamma"],
    )
