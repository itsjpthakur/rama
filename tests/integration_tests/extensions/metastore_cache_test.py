...
from rama.extensions.metastore_cache import RamaMetastoreCache
from rama.key_value.exceptions import KeyValueCodecEncodeException
from rama.key_value.types import (
    JsonKeyValueCodec,
    KeyValueCodec,
    PickleKeyValueCodec,
)
...
@pytest.fixture
def cache() -> RamaMetastoreCache:
    return RamaMetastoreCache(
        namespace=NAMESPACE,
        default_timeout=600,
...
def test_caching_flow(app_context: AppContext, cache: RamaMetastoreCache) -> None:
...
def test_expiry(app_context: AppContext, cache: RamaMetastoreCache) -> None:
