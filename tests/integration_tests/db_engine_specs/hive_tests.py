...
from rama.db_engine_specs.hive import HiveEngineSpec, upload_to_s3
...
def test_df_to_csv() -> None:
    with pytest.raises(RamaException):
...
@mock.patch("rama.db_engine_specs.hive.g", spec={})
def test_df_to_sql_if_exists_fail(mock_g):
    mock_g.user = True
    mock_database = mock.MagicMock()
    mock_database.get_df.return_value.empty = False
    with pytest.raises(RamaException, match="Table already exists"):
        HiveEngineSpec.df_to_sql(
...
@mock.patch("rama.db_engine_specs.hive.g", spec={})
@mock.patch("rama.db_engine_specs.hive.upload_to_s3")
@unittest.skipUnless(
    RamaTestCase.is_module_installed("boto3"), "boto3 not installed"
)
...
@mock.patch("rama.db_engine_specs.base.BaseEngineSpec.fetch_data")
...
@mock.patch("rama.db_engine_specs.hive.HiveEngineSpec._latest_partition_from_df")
...
@mock.patch("rama.db_engine_specs.presto.PrestoEngineSpec.latest_partition")
...
@mock.patch("rama.db_engine_specs.hive.HiveEngineSpec.get_view_names")
