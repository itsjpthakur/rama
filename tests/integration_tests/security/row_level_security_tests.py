...
from rama import db, security_manager, app  # noqa: F401
...
class TestRowLevelSecurity(RamaTestCase):
    """
...
class TestRowLevelSecurityCreateAPI(RamaTestCase):
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
...
class TestRowLevelSecurityUpdateAPI(RamaTestCase):
...
class TestRowLevelSecurityDeleteAPI(RamaTestCase):
    def test_invalid_id_failure(self):
        self.login(ADMIN_USERNAME)
...
class TestRowLevelSecurityWithRelatedAPI(RamaTestCase):
    ...
    @pytest.mark.usefixtures("load_energy_table_with_slice")
    @mock.patch(
        "rama.row_level_security.api.RLSRestApi.base_related_field_filters",
...
class GuestTokenRowLevelSecurityTests(RamaTestCase):
    query_obj: dict[str, Any] = dict(  # noqa: C408
        groupby=[],
        metrics=None,
        filter=[],
