...
from rama import db, is_feature_enabled, security_manager
from rama.extensions import machine_auth_provider_factory
from rama.models.dashboard import Dashboard
...
from rama.utils.webdriver import WebDriverSelenium
...
class TestThumbnailsSeleniumLive(LiveServerTestCase):
    ...
    def test_get_async_dashboard_screenshot(self):
        ...
        """
        with patch("rama.dashboards.api.DashboardRestApi.get") as mock_get:  # noqa: F841
...
class TestWebDriverScreenshotErrorDetector(RamaTestCase):
    @patch("rama.utils.webdriver.WebDriverWait")
    @patch("rama.utils.webdriver.firefox")
    @patch("rama.utils.webdriver.WebDriverSelenium.find_unexpected_errors")
    def test_not_call_find_unexpected_errors_if_feature_disabled(
        ...
        url = get_url_path("Rama.dashboard", dashboard_id_or_slug=1)
...
class TestWebDriverSelenium(RamaTestCase):
    @patch("rama.utils.webdriver.WebDriverWait")
    @patch("rama.utils.webdriver.firefox")
    @patch("rama.utils.webdriver.sleep")
    def test_screenshot_selenium_headstart(
        ...
        user = security_manager.get_user_by_username(ADMIN_USERNAME)
        url = get_url_path("Rama.slice", slice_id=1, standalone="true")
...
class TestThumbnails(RamaTestCase):
    ...
    def test_get_async_dashboard_screenshot_as_fixed_user(self):
        ...
        with (
            patch.dict(
                "rama.thumbnails.digest.current_app.config",
                {
                    "THUMBNAIL_EXECUTORS": [FixedExecutor(ADMIN_USERNAME)],
                },
            ),
            patch(
                "rama.thumbnails.digest._adjust_string_for_executor"
