...
import rama.cli.importexport
import rama.cli.thumbnails
from rama import app, db
...
def test_export_dashboards_versioned_export(app_context, fs):
    ...
    import rama.cli.importexport  # noqa: F811

    # reload to define export_dashboards correctly based on the
    # feature flags
    importlib.reload(rama.cli.importexport)

    runner = app.test_cli_runner()
    with freeze_time("2021-01-01T00:00:00Z"):
        response = runner.invoke(rama.cli.importexport.export_dashboards, ())
...
@mock.patch(
    "rama.commands.dashboard.export.ExportDashboardsCommand.run",
...
def test_export_datasources_versioned_export(app_context, fs):
    ...
    runner = app.test_cli_runner()
    with freeze_time("2021-01-01T00:00:00Z"):
        response = runner.invoke(rama.cli.importexport.export_datasources, ())
...
@mock.patch(
    "rama.commands.dashboard.export.ExportDatasetsCommand.run",
    side_effect=Exception(),
)
...
@mock.patch("rama.commands.dashboard.importers.dispatcher.ImportDashboardsCommand")
def test_import_dashboards_versioned_export(import_dashboards_command, app_context, fs):
    ...
    response = runner.invoke(
        rama.cli.importexport.import_dashboards,
...
@mock.patch(
    "rama.commands.dashboard.importers.dispatcher.ImportDashboardsCommand.run",
    side_effect=Exception(),
)
...
@mock.patch("rama.commands.dataset.importers.dispatcher.ImportDatasetsCommand")
def test_import_datasets_versioned_export(import_datasets_command, app_context, fs):
    ...
    runner = app.test_cli_runner()
    response = runner.invoke(
        rama.cli.importexport.import_datasources, ("-p", "datasets.yaml")
    )
    ...
    runner = app.test_cli_runner()
    response = runner.invoke(
        rama.cli.importexport.import_datasources, ("-p", "datasets.zip")
    )
...
@mock.patch(
    "rama.commands.dataset.importers.dispatcher.ImportDatasetsCommand.run",
    side_effect=Exception(),
)
...
@pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
@mock.patch("rama.tasks.thumbnails.cache_dashboard_thumbnail")
def test_compute_thumbnails(thumbnail_mock, app_context, fs):
    ...
    response = runner.invoke(
        rama.cli.thumbnails.compute_thumbnails,
        ["-d", "-i", dashboard.id],
    )
