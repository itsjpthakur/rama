...
from rama.connectors.sqla.models import (  # noqa: F401
    SqlaTable,
    SqlMetric,
    TableColumn,
)
from rama.constants import CacheRegion
from rama.daos.exceptions import DatasourceNotFound, DatasourceTypeNotSupportedError
from rama.exceptions import RamaGenericDBErrorException
from rama.models.core import Database
from rama.utils import json
from rama.utils.core import backend, get_example_default_schema  # noqa: F401
from rama.utils.database import (  # noqa: F401
    get_example_database,
    get_main_database,
)
from tests.integration_tests.base_tests import db_insert_temp_object, RamaTestCase
...
class TestDatasource(RamaTestCase):
    def setUp(self):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch("rama.connectors.sqla.models.SqlaTable.external_metadata")
    def test_external_metadata_error_return_400(self, mock_get_datasource):
        self.login(ADMIN_USERNAME)
        tbl = self.get_table(name="birth_names")
        url = f"/datasource/external_metadata/table/{tbl.id}/"

        mock_get_datasource.side_effect = RamaGenericDBErrorException("oops")

        pytest.raises(
            RamaGenericDBErrorException,
    ...
    def test_save_default_endpoint_validation_success(self):
        ...
        datasource_post["id"] = tbl_id
        datasource_post["owners"] = [1]
        datasource_post["default_endpoint"] = "http://localhost/rama/1"
        data = dict(data=json.dumps(datasource_post))  # noqa: C408
