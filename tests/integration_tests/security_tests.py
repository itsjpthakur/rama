...
from rama.daos.datasource import DatasourceDAO  # noqa: F401
from rama.models.dashboard import Dashboard
from rama import app, appbuilder, db, security_manager, viz
...
class TestRolePermission(RamaTestCase):
    ...
    def test_gamma_user_schema_access_to_dashboards(self):
        ...
        assert "/rama/dashboard/world_health/" in data
        assert "/rama/dashboard/births/" not in data
    ...
    def assert_can_gamma(self, perm_set):
        ...
        assert ("can_csv", "Rama") in perm_set
        assert ("can_dashboard", "Rama") in perm_set
        assert ("can_explore", "Rama") in perm_set
        assert ("can_share_chart", "Rama") in perm_set
        assert ("can_share_dashboard", "Rama") in perm_set
        assert ("can_explore_json", "Rama") in perm_set
        assert ("can_explore_json", "Rama") in perm_set
    ...
    @unittest.skipUnless(
        RamaTestCase.is_module_installed("pydruid"), "pydruid not installed"
    )
    ...
    def test_gamma_permissions(self):
        ...
        assert ("can_csv", "Rama") in gamma_perm_set
        assert ("can_dashboard", "Rama") in gamma_perm_set
        assert ("can_explore", "Rama") in gamma_perm_set
        assert ("can_share_chart", "Rama") in gamma_perm_set
        assert ("can_share_dashboard", "Rama") in gamma_perm_set
        assert ("can_explore_json", "Rama") in gamma_perm_set
...
class TestSecurityManager(RamaTestCase):
    """
    Testing the Security Manager.
    """

    @patch("rama.security.RamaSecurityManager.raise_for_access")
    def test_can_access_datasource(self, mock_raise_for_access):
        ...
        mock_raise_for_access.side_effect = RamaSecurityException(
            RamaError(
                "dummy",
    ...
    def test_can_access_table(self, mock_raise_for_access):
        ...
        mock_raise_for_access.side_effect = RamaSecurityException(
            RamaError(
                "dummy", RamaErrorType.TABLE_SECURITY_ACCESS_ERROR, ErrorLevel.ERROR
            )
        )

        assert not security_manager.can_access_table(database, table)

    @patch("rama.security.RamaSecurityManager.is_owner")
    @patch("rama.security.RamaSecurityManager.can_access")
    @patch("rama.security.RamaSecurityManager.can_access_schema")
    def test_raise_for_access_datasource(
        ...
        with self.assertRaises(RamaSecurityException):  # noqa: PT027
            security_manager.raise_for_access(datasource=datasource)
...
class TestDatasources(RamaTestCase):
    @patch("rama.security.RamaSecurityManager.can_access_database")
    @patch("rama.security.RamaSecurityManager.get_session")
...
class TestGuestTokens(RamaTestCase):
    ...
    @patch("rama.security.RamaSecurityManager._get_current_epoch_time")
