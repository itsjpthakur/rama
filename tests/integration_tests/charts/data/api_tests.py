...
from tests.integration_tests.base_tests import RamaTestCase, test_client
...
from rama.connectors.sqla.models import TableColumn, SqlaTable
from rama.errors import RamaErrorType
from rama.extensions import async_query_manager_factory, db
from rama.models.annotations import AnnotationLayer
from rama.rama_typing import AdhocColumn
...
class BaseTestChartDataApi(RamaTestCase):
...
class TestPostChartDataApi(BaseTestChartDataApi):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch("rama.utils.decorators.g")
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch(
        "rama.common.query_context_factory.config",
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch(
        "rama.utils.core.current_app.config",
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch(
        "rama.common.query_actions.config",
...
class TestGetChartDataApi(BaseTestChartDataApi):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @with_feature_flags(GLOBAL_ASYNC_QUERIES=True)
    @mock.patch("rama.charts.data.api.QueryContextCacheLoader")
...
@mock.patch(
    "rama.common.query_context_processor.config",
    {
        **app.config,
