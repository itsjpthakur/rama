...
class TestQueryApi(RamaTestCase):
    def insert_query(
        self,
        database_id: int,
        user_id: int,
    ...
    def test_get_query_no_data_access(self):
        ...
        gamma1 = self.create_user(
            "gamma_1", "password", "Gamma", email="gamma1@rama.org"
        )
        gamma2 = self.create_user(
            "gamma_2", "password", "Gamma", email="gamma2@rama.org"
        )
    ...
    @mock.patch("rama.sql_lab.cancel_query")
    @mock.patch("rama.views.core.db.session")
    def test_stop_query_not_found(
        self, mock_rama_db_session, mock_sql_lab_cancel_query
    ):
        """
        ...
        self.login(ADMIN_USERNAME)
        mock_rama_db_session.query().filter_by().one_or_none = query_mock
    ...
    @mock.patch("rama.sql_lab.cancel_query")
    @mock.patch("rama.views.core.db.session")
    def test_stop_query(self, mock_rama_db_session, mock_sql_lab_cancel_query):
        """
        Handles stop query when the DB engine spec does not
        ...
        self.login(ADMIN_USERNAME)
        mock_rama_db_session.query().filter_by().one_or_none().return_value = (
            query_mock
        )
        mock_sql_lab_cancel_query.return_value = True
        rv = self.client.post(
