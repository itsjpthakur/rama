...
from rama.rama_typing import OAuth2ClientConfig
from rama.utils import json
from rama.utils.oauth2 import decode_oauth2_state

if TYPE_CHECKING:
    from rama.db_engine_specs.base import OAuth2State
...
def test_validate_parameters_simple() -> None:
    from rama.db_engine_specs.gsheets import (
        GSheetsEngineSpec,
...
def test_validate_parameters_catalog(
    ...
    g = mocker.patch("rama.db_engine_specs.gsheets.g")
    g.user.email = "admin@example.com"

    create_engine = mocker.patch("rama.db_engine_specs.gsheets.create_engine")
...
def test_upload_new(mocker: MockerFixture) -> None:
    ...
    mocker.patch("rama.db_engine_specs.gsheets.db")
...
def test_upload_existing(mocker: MockerFixture) -> None:
    ...
    with pytest.raises(RamaException) as excinfo:
...
def test_get_url_for_impersonation_username(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "rama.db_engine_specs.gsheets.security_manager.find_user",
        return_value=user,
    )
...
def test_is_oauth2_enabled_no_config(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "rama.db_engine_specs.base.current_app.config",
