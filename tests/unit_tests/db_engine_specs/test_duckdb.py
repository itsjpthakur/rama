...
from rama.config import VERSION_STRING
...
def test_convert_dttm(
    ...
    from rama.db_engine_specs.duckdb import DuckDBEngineSpec as spec  # noqa: N813
...
def test_get_extra_params(mocker: MockerFixture) -> None:
    ...
    """
    from rama.db_engine_specs.duckdb import DuckDBEngineSpec
    ...
    assert DuckDBEngineSpec.get_extra_params(database) == {
        "engine_params": {
            "connect_args": {
                "config": {"custom_user_agent": f"apache-rama/{VERSION_STRING}"}
            }
        }
    }
    ...
    assert DuckDBEngineSpec.get_extra_params(database) == {
        "engine_params": {
            "connect_args": {
                "config": {
                    "custom_user_agent": f"apache-rama/{VERSION_STRING} my-app"
                }
            }
        }
    }
...
def test_build_sqlalchemy_uri() -> None:
    ...
    from rama.db_engine_specs.duckdb import DuckDBEngineSpec, DuckDBParametersType
...
def test_md_build_sqlalchemy_uri() -> None:
    ...
    from rama.db_engine_specs.duckdb import (
