...
import rama.config
from rama.constants import QUERY_CANCEL_KEY, QUERY_EARLY_CANCEL_KEY, USER_AGENT
from rama.db_engine_specs.exceptions import (
    RamaDBAPIConnectionError,
    RamaDBAPIDatabaseError,
    RamaDBAPIOperationalError,
    RamaDBAPIProgrammingError,
)
...
def test_get_extra_params(extra: dict[str, Any], expected: dict[str, Any]) -> None:
    from rama.db_engine_specs.trino import TrinoEngineSpec
...
@patch("rama.utils.core.create_ssl_cert_file")
...
def test_auth_custom_auth() -> None:
    ...
    with patch.dict(
        "rama.config.ALLOWED_EXTRA_AUTHENTICATIONS",
...
def test_get_column_spec(
    ...
    from rama.db_engine_specs.trino import TrinoEngineSpec as spec  # noqa: N813
...
@patch("rama.db_engine_specs.trino.TrinoEngineSpec.cancel_query")
@patch("sqlalchemy.engine.Engine.connect")
...
def test_execute_with_cursor_in_parallel(app, mocker: MockerFixture):
    ...
    with app.test_request_context("/some/place/"):
        ...
        with patch.dict(
            "rama.config.DISALLOWED_SQL_FUNCTIONS",
            {},
            clear=True,
        ):
...
def test_get_dbapi_exception_mapping():
    ...
    assert mapping.get(TrinoUserError) == RamaDBAPIProgrammingError
    assert mapping.get(TrinoInternalError) == RamaDBAPIDatabaseError
    assert mapping.get(TrinoExternalError) == RamaDBAPIOperationalError
    assert mapping.get(RequestsConnectionError) == RamaDBAPIConnectionError
...
@patch("rama.db_engine_specs.trino.TrinoEngineSpec.latest_partition")
@pytest.mark.parametrize(
...
def test_get_oauth2_token(
    ...
    requests = mocker.patch("rama.db_engine_specs.base.requests")
