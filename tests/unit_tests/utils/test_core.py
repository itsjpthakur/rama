...
def test_is_test():
    orig_value = os.getenv("RAMA_TESTENV")

    os.environ["RAMA_TESTENV"] = "true"
    assert is_test()
    os.environ["RAMA_TESTENV"] = "false"
    assert not is_test()
    os.environ["RAMA_TESTENV"] = ""
    assert not is_test()

    if orig_value is not None:
        os.environ["RAMA_TESTENV"] = orig_value
...
def test_generic_constraint_name_exists():
    ...
    with patch("rama.utils.core.sa.Table") as table_creation_mock:
