...
def test_safe_proxy_lambda() -> None:
    ...
    Should raise ``RamaTemplateException``.
    """

    def func(input_: Any) -> Any:
        return input_

    with pytest.raises(RamaTemplateException):
...
def test_safe_proxy_nested_lambda() -> None:
    """
    Test the ``safe_proxy`` helper with a function returning a ``dict``
    containing ``lambda`` value. Should raise ``RamaTemplateException``.
    """
...
def test_user_macros(mocker: MockerFixture):
    ...
    Test all user macros:
        ...
        - ``current_user_email``
    """
    mock_g = mocker.patch("rama.utils.core.g")
    mock_cache_key_wrapper = mocker.patch(
        "rama.jinja_context.ExtraCache.cache_key_wrapper"
    )
    mock_g.user.id = 1
...
def test_dataset_macro(mocker: MockerFixture) -> None:
    ...
    """
    mocker.patch(
        "rama.connectors.sqla.models.security_manager.get_guest_rls_filters",
        return_value=[],
    )
    ...
    DatasetDAO = mocker.patch("rama.daos.dataset.DatasetDAO")  # noqa: N806
...
def test_metric_macro_with_dataset_id(mocker: MockerFixture) -> None:
    ...
    """
    mock_get_form_data = mocker.patch("rama.views.utils.get_form_data")
    DatasetDAO = mocker.patch("rama.daos.dataset.DatasetDAO")  # noqa: N806
...
def test_metric_macro_recursive(mocker: MockerFixture) -> None:
    ...
    """
    mock_g = mocker.patch("rama.jinja_context.g")
...
def test_metric_macro_recursive_cyclic(mocker: MockerFixture) -> None:
    ...
    with pytest.raises(RamaTemplateException) as excinfo:
        metric_macro("c", 1)
...
def test_metric_macro_no_dataset_id_with_context_chart_id(
    ...
    """
    ChartDAO = mocker.patch("rama.daos.chart.ChartDAO")  # noqa: N806
