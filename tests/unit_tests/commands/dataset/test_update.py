...
def test_update_uniqueness_error(mocker: MockerFixture) -> None:
    ...
    mock_g = mocker.patch("rama.security.manager.g")
    mock_g.user = MagicMock()

    mocker.patch(
        "rama.views.base.security_manager.can_access_all_datasources",
        return_value=True,
    )

    mocker.patch(
        "rama.commands.dataset.update.security_manager.raise_for_ownership",
        return_value=None,
    )
