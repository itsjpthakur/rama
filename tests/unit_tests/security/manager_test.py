...
from rama.connectors.sqla.models import Database, SqlaTable
from rama.exceptions import RamaSecurityException
from rama.extensions import appbuilder
from rama.models.slice import Slice
from rama.security.manager import (
    query_context_modified,
    RamaSecurityManager,
)
from rama.sql_parse import Table
from rama.rama_typing import AdhocColumn, AdhocMetric
from rama.utils.core import DatasourceName, override_user
...
def test_security_manager(app_context: None) -> None:
    ...
    """
    sm = RamaSecurityManager(appbuilder)
    assert sm
...
def test_raise_for_access_guest_user_tampered_id(
    ...
    query_context.queries = [QueryObject(metrics=stored_metrics)]  # type: ignore
    with pytest.raises(RamaSecurityException):
        sm.raise_for_access(query_context=query_context)
...
def test_raise_for_access_query_default_schema(
    ...
    SqlaTable = mocker.patch("rama.connectors.sqla.models.SqlaTable")  # noqa: N806
