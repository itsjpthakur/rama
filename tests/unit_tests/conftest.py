...
from rama.common.chart_data import ChartDataResultType
from rama.common.query_object_factory import QueryObjectFactory
from rama.extensions import appbuilder, feature_flag_manager
from rama.initialization import RamaAppInitializer
...
def get_session(mocker: MockerFixture) -> Callable[[], Session]:
    ...
    def get_session():
        ...
        get_session = mocker.patch(
            "rama.security.RamaSecurityManager.get_session",
        )
        ...
        mocker.patch("rama.db.session", in_memory_session)
...
def app(request: SubRequest) -> Iterator[RamaApp]:
    """
    A fixture that generates a Rama app.
    """
    app = RamaApp(__name__)

    app.config.from_object("rama.config")
    app.config["SQLALCHEMY_DATABASE_URI"] = (
        os.environ.get("RAMA__SQLALCHEMY_DATABASE_URI") or "sqlite://"
    )
    ...
    # ``rama.extensions.appbuilder`` is a singleton, and won't rebuild the
    ...
    app_initializer = RamaAppInitializer(app)
    app_initializer.init_app()

    # reload base views to ensure error handlers are applied to the app
    with app.app_context():
        import rama.views.base

        importlib.reload(rama.views.base)

    return app


@pytest.fixture
def client(app: RamaApp) -> Any:
    with app.test_client() as client:
        yield client


@pytest.fixture(autouse=True)
def app_context(app: RamaApp) -> Iterator[None]:
    """
...
def with_feature_flags(**mock_feature_flags):
    ...
    Usage:
        ...
        class TestYourFeature(RamaTestCase):
