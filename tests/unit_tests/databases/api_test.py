...
from rama import db
from rama.commands.database.uploaders.base import UploadCommand
...
from rama.db_engine_specs.sqlite import SqliteEngineSpec
from rama.errors import ErrorLevel, RamaError, RamaErrorType
from rama.exceptions import OAuth2RedirectError, RamaSecurityException
...
def test_filter_by_uuid(
    ...
    Note: this functionality is not used by the Rama UI, but is needed by 3rd
    party tools that use the Rama API. If this tests breaks, please make sure
...
def test_password_mask(
    ...
    mocker.patch("rama.utils.log.DBEventLogger.log")
...
def test_apply_dynamic_database_filter(
    ...
    with app.app_context():
        ...
        mocker.patch("rama.views.filters.current_app.config", new=original_config)
        # Get filtered list
        response_databases = DatabaseDAO.find_all()
        assert response_databases
...
def test_oauth2_happy_path(
    ...
    """
    from rama.databases.api import DatabaseRestApi
    from rama.models.core import Database, DatabaseUserOAuth2Tokens
    ...
    decode_oauth2_state = mocker.patch("rama.databases.api.decode_oauth2_state")
    decode_oauth2_state.return_value = state

    mocker.patch("rama.databases.api.render_template", return_value="OK")
...
def test_table_metadata_happy_path(
    ...
    mocker.patch("rama.databases.api.DatabaseDAO.find_by_id", return_value=database)
    mocker.patch("rama.databases.api.security_manager.raise_for_access")
...
def test_table_metadata_invalid_database(
    ...
    """
    mocker.patch("rama.databases.api.DatabaseDAO.find_by_id", return_value=None)
        ...
        "errors": [
            {
                ...
                "extra": {
                    "issue_codes": [
                        {
                            "code": 1011,
                            "message": "Issue 1011 - Rama encountered an unexpected error.",  # noqa: E501
                        },
                        {
                            "code": 1036,
...
def test_table_metadata_unauthorized(
    ...
    mocker.patch("rama.databases.api.DatabaseDAO.find_by_id", return_value=database)
    mocker.patch(
        "rama.databases.api.security_manager.raise_for_access",
...
def test_catalogs(
    ...
    DatabaseDAO = mocker.patch("rama.databases.api.DatabaseDAO")  # noqa: N806
    DatabaseDAO.find_by_id.return_value = database

    security_manager = mocker.patch(
        "rama.databases.api.security_manager",
