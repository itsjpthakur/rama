...
from rama.rama_typing import DbapiDescription
...
def test_df_to_records() -> None:
    ...
    results = RamaResultSet(data, cursor_descr, BaseEngineSpec)
...
def test_max_pandas_timestamp(input_, expected) -> None:
    ...
    results = RamaResultSet(input_, cursor_descr, BaseEngineSpec)
    df = results.to_pandas_df()
