...
from rama.sql.parse import Table
...
def test_insert_rls_as_subquery(
    ...
    mocker.patch("rama.sql_parse.get_rls_for_table", new=get_rls_for_table)
...
def test_get_rls_for_table(mocker: MockerFixture) -> None:
    ...
    dataset = mocker.patch("rama.db").session.query().filter().one_or_none()
...
def test_extract_table_references(mocker: MockerFixture) -> None:
    ...
    logger = mocker.patch("rama.sql_parse.logger")
    ...
    logger = mocker.patch("rama.migrations.shared.utils.logger")
    sql = "SELECT * FROM table UNION ALL SELECT * FROM other_table"
...
def test_is_select() -> None:
    ...
    Test `is_select`.
    """
    assert not ParsedQuery("SELECT 1; DROP DATABASE rama").is_select()
    assert ParsedQuery(
