...
from rama.errors import ErrorLevel, RamaErrorType
from rama.exceptions import OAuth2Error, RamaErrorException
from rama.models.core import Database
from rama.sql_lab import execute_sql_statements, get_sql_results
from rama.utils.core import override_user
...
def test_execute_sql_statement(mocker: MockerFixture, app: None) -> None:
    ...
    """
    from rama.sql_lab import execute_sql_statement
    ...
    RamaResultSet = mocker.patch("rama.sql_lab.RamaResultSet")  # noqa: N806
    ...
    RamaResultSet.assert_called_with([(42,)], cursor.description, db_engine_spec)
...
def test_execute_sql_statement_with_rls(
    ...
    RamaResultSet = mocker.patch("rama.sql_lab.RamaResultSet")  # noqa: N806
    mocker.patch(
        "rama.sql_lab.insert_rls_as_subquery",
        return_value=sqlparse.parse("SELECT * FROM sales WHERE organization_id=42")[0],
    )
    mocker.patch("rama.sql_lab.is_feature_enabled", return_value=True)
...
def test_execute_sql_statement_exceeds_payload_limit(mocker: MockerFixture) -> None:
    ...
    mocker.patch("rama.sql_lab.get_query", return_value=query)
    ...
    mocker.patch(
        "rama.sql_lab._serialize_payload", side_effect=mock_serialize_payload
    )

    # Mock db.session.refresh to avoid AttributeError during session refresh
    mocker.patch("rama.sql_lab.db.session.refresh", return_value=None)

    # Mock the results backend to avoid "Results backend is not configured" error
    mocker.patch("rama.sql_lab.results_backend", return_value=True)

    # Test that the exception is raised when the payload exceeds the limit
    with pytest.raises(RamaErrorException):
...
def test_execute_sql_statement_within_payload_limit(mocker: MockerFixture) -> None:
    ...
    except RamaErrorException:
        pytest.fail(
            "RamaErrorException should not have been raised for payload within the limit"  # noqa: E501
        )
...
def test_sql_lab_insert_rls_as_subquery(
    ...
    from rama.connectors.sqla.models import RowLevelSecurityFilter, SqlaTable
    from rama.models.core import Database
    from rama.models.sql_lab import Query
    from rama.security.manager import RamaSecurityManager
    from rama.sql_lab import execute_sql_statement
    from rama.utils.core import RowLevelSecurityFilterType
    ...
    with override_user(admin):
        rama_result_set = execute_sql_statement(
    ...
    assert (
        rama_result_set.to_pandas_df().to_markdown()
        == """
|    |   c |
|---:|----:|
|  0 |   0 |
...
|  4 |   4 |""".strip()
    ...
    db.session.add(rls)
    db.session.flush()
    mocker.patch.object(RamaSecurityManager, "find_user", return_value=admin)
    mocker.patch("rama.sql_lab.is_feature_enabled", return_value=True)
...
def test_get_sql_results_oauth2(mocker: MockerFixture, app) -> None:
    ...
    mocker.patch(
        "rama.db_engine_specs.base.uuid4",
        return_value=UUID("fb11f528-6eba-4a8a-837e-6b0d39ee9187"),
    )

    g = mocker.patch("rama.db_engine_specs.base.g")
