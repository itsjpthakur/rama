...
@with_feature_flags(ENABLE_RAMA_META_DB=True)
def test_rama(mocker: MockerFixture, app_context: None, table1: None) -> None:
    """
    Simple test querying a table.
    """
    mocker.patch("rama.extensions.metadb.security_manager")
...
@with_feature_flags(ENABLE_RAMA_META_DB=True)
def test_rama_limit(mocker: MockerFixture, app_context: None, table1: None) -> None:
    """
    Simple that limit is applied when querying a table.
    """
    mocker.patch(
        "rama.extensions.metadb.current_app.config",
        {
            "DB_SQLA_URI_VALIDATOR": None,
            "RAMA_META_DB_LIMIT": 1,
...
def test_security_manager(
    ...
    mocker.patch(
        "rama.extensions.metadb.security_manager",
        new=security_manager,
    )
    security_manager.raise_for_access.side_effect = RamaSecurityException(
        RamaError(
    ...
    engine = create_engine("rama://")
    conn = engine.connect()
    with pytest.raises(RamaSecurityException) as excinfo:
...
def test_allowed_dbs(mocker: MockerFixture, app_context: None, table1: None) -> None:
    ...
    engine = create_engine("rama://", allowed_dbs=["database1"])
    conn = engine.connect()
